...
void AudioHeap_PopCache(s32 tableType)
{
	...
	persistentPool->cur = (u8*)entryPtr;
...
void AudioHeap_InitMainPools(s32 initPoolSize)
{
	...
	AudioHeap_AllocPoolInit(&gAudioContext.audioSessionPool, gAudioContext.audioHeap + initPoolSize, gAudioContext.audioHeapSize - initPoolSize);
...
void AudioHeap_SessionPoolsInit(AudioPoolSplit4* split)
{
	...
	AudioHeap_AllocPoolInit(&gAudioContext.notesAndBuffersPool, AudioHeap_Alloc(&gAudioContext.audioSessionPool, split->wantSeq), split->wantSeq);
	AudioHeap_AllocPoolInit(&gAudioContext.cachePool, AudioHeap_Alloc(&gAudioContext.audioSessionPool, split->wantCustom), split->wantCustom);
}

void AudioHeap_CachePoolInit(AudioPoolSplit2* split)
{
	gAudioContext.cachePool.cur = gAudioContext.cachePool.start;
	AudioHeap_AllocPoolInit(&gAudioContext.persistentCommonPool, AudioHeap_Alloc(&gAudioContext.cachePool, split->wantPersistent), split->wantPersistent);
	AudioHeap_AllocPoolInit(&gAudioContext.temporaryCommonPool, AudioHeap_Alloc(&gAudioContext.cachePool, split->wantTemporary), split->wantTemporary);
}
...
void AudioHeap_PersistentCachesInit(AudioPoolSplit3* split)
{
	...
	AudioHeap_AllocPoolInit(&gAudioContext.seqCache.persistent.pool, AudioHeap_Alloc(&gAudioContext.persistentCommonPool, split->wantSeq), split->wantSeq);
	AudioHeap_AllocPoolInit(&gAudioContext.fontCache.persistent.pool, AudioHeap_Alloc(&gAudioContext.persistentCommonPool, split->wantFont), split->wantFont);
	AudioHeap_AllocPoolInit(&gAudioContext.sampleBankCache.persistent.pool, AudioHeap_Alloc(&gAudioContext.persistentCommonPool, split->wantSample), split->wantSample);
...
void AudioHeap_TemporaryCachesInit(AudioPoolSplit3* split)
{
	...
	AudioHeap_AllocPoolInit(&gAudioContext.seqCache.temporary.pool, AudioHeap_Alloc(&gAudioContext.temporaryCommonPool, split->wantSeq), split->wantSeq);
	AudioHeap_AllocPoolInit(&gAudioContext.fontCache.temporary.pool, AudioHeap_Alloc(&gAudioContext.temporaryCommonPool, split->wantFont), split->wantFont);
	AudioHeap_AllocPoolInit(&gAudioContext.sampleBankCache.temporary.pool, AudioHeap_Alloc(&gAudioContext.temporaryCommonPool, split->wantSample), split->wantSample);
...
void* AudioHeap_AllocCached(s32 tableType, s32 size, s32 cache, s32 id)
{
	...
	if(cache == CACHE_TEMPORARY)
	{
		...
		if(tableType == FONT_TABLE)
		{
			if(firstVal == 4)
			{
				for(i = 0; i < gAudioContext.numNotes; i++)
				{
					if(gAudioContext.notes[i].playbackState.fontId == tp->entries[0].id && gAudioContext.notes[i].noteSubEu.bitField0.enabled != 0)
					{
						break;
					}
				}
			...
			if(secondVal == 4)
			{
				for(i = 0; i < gAudioContext.numNotes; i++)
				{
					if(gAudioContext.notes[i].playbackState.fontId == tp->entries[1].id && gAudioContext.notes[i].noteSubEu.bitField0.enabled != 0)
					{
						break;
					}
				}
		...
		else
		{
			...
			if(tableType == SEQUENCE_TABLE)
			{
				if(firstVal == 2)
				{
					for(i = 0; i < gAudioContext.audioBufferParameters.numSequencePlayers; i++)
					{
						if(gAudioContext.seqPlayers[i].enabled != 0 && gAudioContext.seqPlayers[i].seqId == tp->entries[0].id)
						{
							break;
						}
					}
				...
				if(secondVal == 2)
				{
					for(i = 0; i < gAudioContext.audioBufferParameters.numSequencePlayers; i++)
					{
						if(gAudioContext.seqPlayers[i].enabled != 0 && gAudioContext.seqPlayers[i].seqId == tp->entries[1].id)
						{
							break;
						}
					}
	...
	loadedPool->persistent.entries[loadedPool->persistent.numEntries].ptr = (u8*)mem;
	...
	switch(gAudioContext.resetStatus)
	{
		...
		case 4:
			...
			else
			{
				for(i = 0; i < gAudioContext.numNotes; i++)
				{
					if(gAudioContext.notes[i].noteSubEu.bitField0.enabled && gAudioContext.notes[i].playbackState.adsr.action.s.state != ADSR_STATE_DISABLED)
					{
						gAudioContext.notes[i].playbackState.adsr.fadeOutVel = gAudioContext.audioBufferParameters.updatesPerFrameInv;
...
void AudioHeap_Init(void)
{
	...
	gAudioContext.audioBufferParameters.samplesPerFrameTarget = ((gAudioContext.audioBufferParameters.frequency / gAudioContext.refreshRate) + 0xF) & 0xFFF0;
	gAudioContext.audioBufferParameters.minAiBufferLength = gAudioContext.audioBufferParameters.samplesPerFrameTarget - 0x10;
	gAudioContext.audioBufferParameters.maxAiBufferLength = gAudioContext.audioBufferParameters.samplesPerFrameTarget + 0x10;
	gAudioContext.audioBufferParameters.updatesPerFrame = ((gAudioContext.audioBufferParameters.samplesPerFrameTarget + 0x10) / 0xD0) + 1;
	gAudioContext.audioBufferParameters.samplesPerUpdate = (gAudioContext.audioBufferParameters.samplesPerFrameTarget / gAudioContext.audioBufferParameters.updatesPerFrame) & ~7;
	...
	gAudioContext.audioBufferParameters.unkUpdatesPerFrameScaled = (1.0f / 256.0f) / gAudioContext.audioBufferParameters.updatesPerFrame;
	...
	gAudioContext.tempoInternalToExternal = (u32)(gAudioContext.audioBufferParameters.updatesPerFrame * 2880000.0f / gTatumsPerBeat / gAudioContext.unk_2960);
	...
	gAudioContext.maxAudioCmds = gAudioContext.numNotes * 0x10 * gAudioContext.audioBufferParameters.updatesPerFrame + spec->numReverbs * 0x18 + 0x140;
	...
	gAudioContext.notes = (Note*)AudioHeap_AllocZeroed(&gAudioContext.notesAndBuffersPool, gAudioContext.numNotes * sizeof(Note));
	Audio_NoteInitAll();
	Audio_InitNoteFreeList();
	gAudioContext.noteSubsEu = (NoteSubEu*)AudioHeap_AllocZeroed(&gAudioContext.notesAndBuffersPool, gAudioContext.audioBufferParameters.updatesPerFrame * gAudioContext.numNotes * sizeof(NoteSubEu));

	for(i = 0; i != 2; i++)
	{
		gAudioContext.abiCmdBufs[i] = (Acmd*)AudioHeap_AllocDmaMemoryZeroed(&gAudioContext.notesAndBuffersPool, gAudioContext.maxAudioCmds * sizeof(u64));
	}

	gAudioContext.unk_3520 = (f32*)AudioHeap_Alloc(&gAudioContext.notesAndBuffersPool, 0x100 * sizeof(f32));
	func_800DDE3C();
	for(i = 0; i < 4; i++)
	{
	...
	for(i = 0; i < gAudioContext.numSynthesisReverbs; i++)
	{
		...
		reverb->useReverb = 8;
		reverb->leftRingBuf = (s16*)AudioHeap_AllocZeroedAttemptExternal(&gAudioContext.notesAndBuffersPool, reverb->windowSize * sizeof(s16) * 2);  // TODO FIX HACK
		reverb->rightRingBuf = (s16*)AudioHeap_AllocZeroedAttemptExternal(&gAudioContext.notesAndBuffersPool, reverb->windowSize * sizeof(s16) * 2); // TODO FIX HACK
		...
		if(reverb->downsampleRate != 1)
		{
			...
			{
				mem = (s16*)AudioHeap_AllocZeroedAttemptExternal(&gAudioContext.notesAndBuffersPool, 0x340);
		...
		{
			reverb->filterLeftState = (s16*)AudioHeap_AllocDmaMemoryZeroed(&gAudioContext.notesAndBuffersPool, 0x40);
			reverb->filterLeft = (s16*)AudioHeap_AllocDmaMemory(&gAudioContext.notesAndBuffersPool, 8 * sizeof(s16));
		...
		{
			reverb->filterRightState = (s16*)AudioHeap_AllocDmaMemoryZeroed(&gAudioContext.notesAndBuffersPool, 0x40);
			reverb->filterRight = (s16*)AudioHeap_AllocDmaMemory(&gAudioContext.notesAndBuffersPool, 8 * sizeof(s16));
...
void* AudioHeap_AllocPermanent(s32 tableType, s32 id, u32 size)
{
	...
	gAudioContext.permanentCache[index].ptr = (u8*)ret;
	if(ret == NULL)
	{
		return NULL;
	}
...
void AudioHeap_DiscardSampleCacheEntry(SampleCacheEntry* entry)
{
	...
	for(fontId = 0; fontId < numFonts; fontId++)
	{
		...
		if(((sampleBankId1 != 0xFF) && (entry->sampleBankId == sampleBankId1)) || ((sampleBankId2 != 0xFF) && (entry->sampleBankId == sampleBankId2)) || entry->sampleBankId == 0)
		{
...
void AudioHeap_DiscardSampleCaches(void)
{
	...
	for(fontId = 0; fontId < numFonts; fontId++)
	{
		...
		{
			AudioHeap_DiscardSampleCacheForFont(&gAudioContext.persistentSampleCache.entries[j], sampleBankId1, sampleBankId2, fontId);
		}
		for(j = 0; j < gAudioContext.temporarySampleCache.size; j++)
		{
			AudioHeap_DiscardSampleCacheForFont(&gAudioContext.temporarySampleCache.entries[j], sampleBankId1, sampleBankId2, fontId);
		}
	}
}
...
void AudioHeap_ApplySampleBankCacheInternal(s32 apply, s32 sampleBankId)
{
	...
	for(fontId = 0; fontId < numFonts; fontId++)
	{
		...
		if((sampleBankId1 != 0xFF) || (sampleBankId2 != 0xFF))
		{
			if(!AudioLoad_IsFontLoadComplete(fontId) || AudioHeap_SearchCaches(FONT_TABLE, CACHE_EITHER, fontId) == NULL)
			{
				continue;
			}
