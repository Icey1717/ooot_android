...
bool cmd_time_settings(GlobalContext* globalCtx, const SceneCmd* cmd)
{
	...
	{
		gSaveContext.skyboxTime = gSaveContext.dayTime = ((cmd->timeSettings.hour + (cmd->timeSettings.min / 60.0f)) * 60.0f) / ((f32)(24 * 60) / 0x10000);
	}
	...
	if(((globalCtx->envCtx.timeIncrement == 0) && (gSaveContext.cutsceneIndex < 0xFFF0)) || (gSaveContext.entranceIndex == 0x0604))
	{
...
bool cmd_alternate_headers(GlobalContext* globalCtx, const SceneCmd* cmd)
{
	...
	if(gSaveContext.sceneSetupIndex != 0)
	{
		...
		else
		{
			...
			{
				altHeader = ((SceneCmd**)SEGMENTED_TO_VIRTUAL(cmd->altHeaders.segment))[gSaveContext.sceneSetupIndex - 2];
...
bool cmd_misc_settings(GlobalContext* globalCtx, const SceneCmd* cmd)
{
	...
	if(((globalCtx->sceneNum >= SCENE_SPOT00) && (globalCtx->sceneNum <= SCENE_GANON_TOU)) || ((globalCtx->sceneNum >= SCENE_ENTRA) && (globalCtx->sceneNum <= SCENE_SHRINE_R)))
	{
