...
#define LINK_ANIMATION_OFFSET(addr, offset) (((uintptr_t)addr) + ((uintptr_t)offset))
...
void SkelAnime_DrawLimbLod(GlobalContext* globalCtx, s32 limbIndex, void** skeleton, Vec3s* jointTable, OverrideLimbDrawOpa overrideLimbDraw, PostLimbDrawOpa postLimbDraw, void* arg, s32 lod)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 773);
	...
	if((overrideLimbDraw == NULL) || !overrideLimbDraw(globalCtx, limbIndex, &dList, &pos, &rot, arg))
	{
		...
		if(dList != NULL)
		{
			gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_skelanime.c", 805), G_MTX_LOAD);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 821);
}
...
void SkelAnime_DrawLod(GlobalContext* globalCtx, void** skeleton, Vec3s* jointTable, OverrideLimbDrawOpa overrideLimbDraw, PostLimbDrawOpa postLimbDraw, void* arg, s32 lod)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 849);
	...
	if((overrideLimbDraw == NULL) || !overrideLimbDraw(globalCtx, 1, &dList, &pos, &rot, arg))
	{
		...
		if(dList != NULL)
		{
			gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_skelanime.c", 881), G_MTX_LOAD);
			gSPDisplayList(POLY_OPA_DISP++, dList);
		}
	}
	if(postLimbDraw != NULL)
	...
	{
		SkelAnime_DrawLimbLod(globalCtx, rootLimb->child, skeleton, jointTable, overrideLimbDraw, postLimbDraw, arg, lod);
	}

	Matrix_Pop();

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 894);
}
...
void SkelAnime_DrawFlexLimbLod(GlobalContext* globalCtx, s32 limbIndex, void** skeleton, Vec3s* jointTable, OverrideLimbDrawOpa overrideLimbDraw, PostLimbDrawOpa postLimbDraw, void* arg, s32 lod, Mtx** mtx)
{
	...
	if((overrideLimbDraw == NULL) || !overrideLimbDraw(globalCtx, limbIndex, &newDList, &pos, &rot, arg))
	{
		...
		if(newDList != NULL)
		{
			...
			{
				OPEN_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 946);
				gSPMatrix(POLY_OPA_DISP++, *mtx, G_MTX_LOAD);
				gSPDisplayList(POLY_OPA_DISP++, newDList);
				CLOSE_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 949);
			}
			(*mtx)++;
		}
	...
	if(limb->child != LIMB_DONE)
	{
		SkelAnime_DrawFlexLimbLod(globalCtx, limb->child, skeleton, jointTable, overrideLimbDraw, postLimbDraw, arg, lod, mtx);
	}
	...
	if(limb->sibling != LIMB_DONE)
	{
		SkelAnime_DrawFlexLimbLod(globalCtx, limb->sibling, skeleton, jointTable, overrideLimbDraw, postLimbDraw, arg, lod, mtx);
	}
}
...
void SkelAnime_DrawFlexLod(GlobalContext* globalCtx, void** skeleton, Vec3s* jointTable, s32 dListCount, OverrideLimbDrawOpa overrideLimbDraw, PostLimbDrawOpa postLimbDraw, void* arg, s32 lod)
{
	...
	Vec3f pos;
	Vec3s rot;
	Mtx* mtx = (Mtx*)Graph_Alloc(globalCtx->gfxCtx, dListCount * sizeof(Mtx));
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 1000);
	...
	if(rootLimb->child != LIMB_DONE)
	{
		SkelAnime_DrawFlexLimbLod(globalCtx, rootLimb->child, skeleton, jointTable, overrideLimbDraw, postLimbDraw, arg, lod, &mtx);
	}

	Matrix_Pop();

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 1053);
}
...
void SkelAnime_DrawLimbOpa(GlobalContext* globalCtx, s32 limbIndex, void** skeleton, Vec3s* jointTable, OverrideLimbDrawOpa overrideLimbDraw, PostLimbDrawOpa postLimbDraw, void* arg)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 1076);
	Matrix_Push();
	...
	if((overrideLimbDraw == NULL) || !overrideLimbDraw(globalCtx, limbIndex, &dList, &pos, &rot, arg))
	{
		...
		if(dList != NULL)
		{
			gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_skelanime.c", 1103), G_MTX_LOAD);
			gSPDisplayList(POLY_OPA_DISP++, dList);
		}
	}
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 1121);
}
...
void SkelAnime_DrawOpa(GlobalContext* globalCtx, void** skeleton, Vec3s* jointTable, OverrideLimbDrawOpa overrideLimbDraw, PostLimbDrawOpa postLimbDraw, void* arg)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 1148);
	...
	if((overrideLimbDraw == NULL) || !overrideLimbDraw(globalCtx, 1, &dList, &pos, &rot, arg))
	{
		...
		if(dList != NULL)
		{
			gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_skelanime.c", 1176), G_MTX_LOAD);
			gSPDisplayList(POLY_OPA_DISP++, dList);
		}
	}
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 1190);
}
...
void SkelAnime_DrawFlexLimbOpa(GlobalContext* globalCtx, s32 limbIndex, void** skeleton, Vec3s* jointTable, OverrideLimbDrawOpa overrideLimbDraw, PostLimbDrawOpa postLimbDraw, void* arg, Mtx** limbMatricies)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 1214);
	...
	if(limb->child != LIMB_DONE)
	{
		SkelAnime_DrawFlexLimbOpa(globalCtx, limb->child, skeleton, jointTable, overrideLimbDraw, postLimbDraw, arg, limbMatricies);
	}
	...
	if(limb->sibling != LIMB_DONE)
	{
		SkelAnime_DrawFlexLimbOpa(globalCtx, limb->sibling, skeleton, jointTable, overrideLimbDraw, postLimbDraw, arg, limbMatricies);
	}
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 1265);
}
...
void SkelAnime_DrawFlexOpa(GlobalContext* globalCtx, void** skeleton, Vec3s* jointTable, s32 dListCount, OverrideLimbDrawOpa overrideLimbDraw, PostLimbDrawOpa postLimbDraw, void* arg)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 1294);
	...
	if(rootLimb->child != LIMB_DONE)
	{
		SkelAnime_DrawFlexLimbOpa(globalCtx, rootLimb->child, skeleton, jointTable, overrideLimbDraw, postLimbDraw, arg, &mtx);
	}

	Matrix_Pop();
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_skelanime.c", 1347);
}
...
void SkelAnime_GetFrameData(AnimationHeader* animation, s32 frame, s32 limbCount, Vec3s* frameTable)
{
	...
	{
		frameTable->x = (jointIndices->x >= staticIndexMax) ? dynamicData[jointIndices->x] : staticData[jointIndices->x];
		frameTable->y = (jointIndices->y >= staticIndexMax) ? dynamicData[jointIndices->y] : staticData[jointIndices->y];
		frameTable->z = (jointIndices->z >= staticIndexMax) ? dynamicData[jointIndices->z] : staticData[jointIndices->z];
	}
}
...
Gfx* SkelAnime_DrawLimb(GlobalContext* globalCtx, s32 limbIndex, void** skeleton, Vec3s* jointTable, OverrideLimbDraw overrideLimbDraw, PostLimbDraw postLimbDraw, void* arg, Gfx* gfx)
{
	...
	if((overrideLimbDraw == NULL) || !overrideLimbDraw(globalCtx, limbIndex, &dList, &pos, &rot, arg, &gfx))
	{
		...
		if(dList != NULL)
		{
			gSPMatrix(gfx++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_skelanime.c", 1489), G_MTX_LOAD);
	...
	if(limb->child != LIMB_DONE)
	{
		gfx = SkelAnime_DrawLimb(globalCtx, limb->child, skeleton, jointTable, overrideLimbDraw, postLimbDraw, arg, gfx);
	}
	...
	if(limb->sibling != LIMB_DONE)
	{
		gfx = SkelAnime_DrawLimb(globalCtx, limb->sibling, skeleton, jointTable, overrideLimbDraw, postLimbDraw, arg, gfx);
	}
...
Gfx* SkelAnime_Draw(GlobalContext* globalCtx, void** skeleton, Vec3s* jointTable, OverrideLimbDraw overrideLimbDraw, PostLimbDraw postLimbDraw, void* arg, Gfx* gfx)
{
	...
	if((overrideLimbDraw == NULL) || !overrideLimbDraw(globalCtx, 1, &dList, &pos, &rot, arg, &gfx))
	{
		...
		if(dList != NULL)
		{
			gSPMatrix(gfx++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_skelanime.c", 1558), G_MTX_LOAD);
			gSPDisplayList(gfx++, dList);
		}
	}
	...
	if(rootLimb->child != LIMB_DONE)
	{
		gfx = SkelAnime_DrawLimb(globalCtx, rootLimb->child, skeleton, jointTable, overrideLimbDraw, postLimbDraw, arg, gfx);
	}
...
Gfx* SkelAnime_DrawFlexLimb(GlobalContext* globalCtx, s32 limbIndex, void** skeleton, Vec3s* jointTable, OverrideLimbDraw overrideLimbDraw, PostLimbDraw postLimbDraw, void* arg, Mtx** mtx, Gfx* gfx)
{
	...
	if(limb->child != LIMB_DONE)
	{
		gfx = SkelAnime_DrawFlexLimb(globalCtx, limb->child, skeleton, jointTable, overrideLimbDraw, postLimbDraw, arg, mtx, gfx);
	}
	...
	if(limb->sibling != LIMB_DONE)
	{
		gfx = SkelAnime_DrawFlexLimb(globalCtx, limb->sibling, skeleton, jointTable, overrideLimbDraw, postLimbDraw, arg, mtx, gfx);
	}
...
Gfx* SkelAnime_DrawFlex(GlobalContext* globalCtx, void** skeleton, Vec3s* jointTable, s32 dListCount, OverrideLimbDraw overrideLimbDraw, PostLimbDraw postLimbDraw, void* arg, Gfx* gfx)
{
	...
	if(rootLimb->child != LIMB_DONE)
	{
		gfx = SkelAnime_DrawFlexLimb(globalCtx, rootLimb->child, skeleton, jointTable, overrideLimbDraw, postLimbDraw, arg, &mtx, gfx);
	}
...
void AnimationContext_Update(GlobalContext* globalCtx, AnimationContext* animationCtx)
{
	static AnimationEntryCallback animFuncs[] = {
	    AnimationContext_LoadFrame, AnimationContext_CopyAll, AnimationContext_Interp, AnimationContext_CopyTrue, AnimationContext_CopyFalse, AnimationContext_MoveActor,
	};
	AnimationEntry* entry;
...
s32 LinkAnimation_Morph(GlobalContext* globalCtx, SkelAnime* skelAnime)
{
	...
	AnimationContext_SetInterp(globalCtx, skelAnime->limbCount, skelAnime->jointTable, skelAnime->morphTable, 1.0f - (skelAnime->morphWeight / prevMorphWeight));
	return 0;
}
...
void LinkAnimation_AnimateFrame(GlobalContext* globalCtx, SkelAnime* skelAnime)
{
	...
	if(skelAnime->morphWeight != 0)
	{
		...
		{
			AnimationContext_SetInterp(globalCtx, skelAnime->limbCount, skelAnime->jointTable, skelAnime->morphTable, skelAnime->morphWeight);
		}
	}
}
...
{
	LinkAnimation_Change(globalCtx, skelAnime, animation, 1.0f, 0.0f, Animation_GetLastFrame(animation), ANIMMODE_ONCE, 0.0f);
}
...
{
	LinkAnimation_Change(globalCtx, skelAnime, animation, playSpeed, 0.0f, Animation_GetLastFrame(animation), ANIMMODE_ONCE, 0.0f);
}
...
{
	LinkAnimation_Change(globalCtx, skelAnime, animation, 1.0f, 0.0f, Animation_GetLastFrame(animation), ANIMMODE_LOOP, 0.0f);
}
...
{
	LinkAnimation_Change(globalCtx, skelAnime, animation, playSpeed, 0.0f, Animation_GetLastFrame(animation), ANIMMODE_LOOP, 0.0f);
}
...
static s32 SkelAnime_Morph(GlobalContext* context, SkelAnime* skelAnime)
{
	...
	SkelAnime_InterpFrameTable(skelAnime->limbCount, skelAnime->jointTable, skelAnime->jointTable, skelAnime->morphTable, 1.0f - (skelAnime->morphWeight / prevMorphWeight));
	return 0;
}
...
static s32 SkelAnime_MorphTaper(GlobalContext* context, SkelAnime* skelAnime)
{
	...
	SkelAnime_InterpFrameTable(skelAnime->limbCount, skelAnime->jointTable, skelAnime->jointTable, skelAnime->morphTable, 1.0f - curWeight);
	return 0;
}
...
void SkelAnime_AnimateFrame(SkelAnime* skelAnime)
{
	...
	if(INTERPOLATE_ANIM || (skelAnime->mode & ANIM_INTERP))
	{
		...
		SkelAnime_InterpFrameTable(skelAnime->limbCount, skelAnime->jointTable, skelAnime->jointTable, nextjointTable, partialFrame);
	}
	if(skelAnime->morphWeight != 0)
	{
		f32 updateRate = FMV2;
		...
		{
			SkelAnime_InterpFrameTable(skelAnime->limbCount, skelAnime->jointTable, skelAnime->jointTable, skelAnime->morphTable, skelAnime->morphWeight);
		}
	}
}
...
static s32 SkelAnime_Once(GlobalContext* context, SkelAnime* skelAnime)
{
	...
	if(skelAnime->curFrame == skelAnime->endFrame)
	{
		SkelAnime_GetFrameData((AnimationHeader*)skelAnime->animation, (s32)skelAnime->curFrame, skelAnime->limbCount, skelAnime->jointTable);
		SkelAnime_AnimateFrame(skelAnime);
		return 1;
	}
