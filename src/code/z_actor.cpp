...
void ActorShadow_Draw(Actor* actor, Lights* lights, GlobalContext* globalCtx, Gfx* dlist, Color_RGBA8* color)
{
	...
	if(actor->floorPoly != NULL)
	{
		...
		if(temp1 >= -50.0f && temp1 < 500.0f)
		{
			...
			if(color != NULL)
			{
				gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, color->r, color->g, color->b, (u32)(actor->shape.shadowAlpha * temp2) & 0xFF);
			}
			else
			{
...
void ActorShadow_DrawFeet(Actor* actor, Lights* lights, GlobalContext* globalCtx)
{
	...
	if(distToFloor < 200.0f)
	{
			...
			*floorHeightPtr = func_800BFCB8(globalCtx, &floorMtx, feetPosPtr);
			...
			if((-1.0f <= distToFloor) && (distToFloor < 500.0f))
			{
				...
				for(j = 0; j < numLights; j++)
				{
					...
					{
						lightNum = (lightPtr->l.col[0] + lightPtr->l.col[1] + lightPtr->l.col[2]) * ABS(lightPtr->l.dir[1]);
						if(lightNum > 0)
						{
							lightNumMax += lightNum;
							ActorShadow_DrawFoot(globalCtx, lightPtr, &floorMtx, lightNum, shadowAlpha, shadowScaleX, shadowScaleZ);
						}
					}
					lightPtr++;
				}
				...
				for(j = 0; j < 2; j++)
				{
					if(lightPtr->l.dir[1] > 0)
					{
						lightNum = ((lightPtr->l.col[0] + lightPtr->l.col[1] + lightPtr->l.col[2]) * ABS(lightPtr->l.dir[1])) - (lightNumMax * 8);
						if(lightNum > 0)
						{
							ActorShadow_DrawFoot(globalCtx, lightPtr, &floorMtx, lightNum, shadowAlpha, shadowScaleX, shadowScaleZ);
						}
		...
		if(!(actor->bgCheckFlags & BG_STATE_0))
		{
			actor->shape.feetFloorFlags = 0;
		}
...
NaviColor sNaviColorList[] = {
    {{0, 255, 0, 255}, {0, 255, 0, 0}},	    {{0, 255, 0, 255}, {0, 255, 0, 0}}, {{255, 255, 255, 255}, {0, 0, 255, 0}}, {{0, 255, 0, 255}, {0, 255, 0, 0}}, {{150, 150, 255, 255}, {150, 150, 255, 0}},
    {{255, 255, 0, 255}, {200, 155, 0, 0}}, {{0, 255, 0, 255}, {0, 255, 0, 0}}, {{0, 255, 0, 255}, {0, 255, 0, 0}},	{{0, 255, 0, 255}, {0, 255, 0, 0}}, {{255, 255, 0, 255}, {200, 155, 0, 0}},
    {{0, 255, 0, 255}, {0, 255, 0, 0}},	    {{0, 255, 0, 255}, {0, 255, 0, 0}}, {{0, 255, 0, 255}, {0, 255, 0, 0}},
};
...
void func_8002C124(TargetContext* targetCtx, GlobalContext* globalCtx)
{
	...
	if((actor != NULL) && !(actor->flags & ACTOR_FLAG_27))
	{
		...
		Matrix_Translate(actor->focus.pos.x, actor->focus.pos.y + (actor->targetArrowOffset * actor->scale.y) + 17.0f, actor->focus.pos.z, MTXMODE_NEW);
...
void TitleCard_Draw(GlobalContext* globalCtx, TitleCardContext* titleCtx)
{
	...
	if(titleCtx->alpha != 0)
	{
		...
		gDPSetPrimColor(OVERLAY_DISP++, 0, 0, (u8)titleCtx->intensity, (u8)titleCtx->intensity, (u8)titleCtx->intensity, (u8)titleCtx->alpha);

		gDPLoadTextureBlock(OVERLAY_DISP++, (s32)titleCtx->texture + spB0, G_IM_FMT_IA, G_IM_SIZ_8b, spCC, spC8, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);

		gSPTextureRectangle(OVERLAY_DISP++, spC0, spB8, ((sp38 * 2) + spC0) - 4, spB8 + (spC8 * 4) - 1, G_TX_RENDERTILE, 0, 0, 1 << 10, 1 << 10);

		spC8 = titleCtx->height - spC8;

		if(spC8 > 0)
		{
			gDPLoadTextureBlock(OVERLAY_DISP++, (s32)titleCtx->texture + spB0 + 0x1000, G_IM_FMT_IA, G_IM_SIZ_8b, spCC, spC8, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
...
f32 func_8002DCE4(Player* player)
{
	...
	else if(player->stateFlags1 & PLAYER_STATE_SWIMMING)
	{ // When swimming
...
{
	if((actor->bgCheckFlags & BG_STATE_0) && (arg1 < -11.0f))
	{
		actor->bgCheckFlags &= ~BG_STATE_0;
		actor->bgCheckFlags |= BG_STATE_2;
...
s32 func_8002E2AC(GlobalContext* globalCtx, Actor* actor, Vec3f* arg2, s32 arg3)
{
	...
	{ // actor is above ground
		if((actor->bgCheckFlags & BG_STATE_0) && (floorHeightDiff >= -11.0f))
		{
			func_80043334(&globalCtx->colCtx, actor, actor->floorBgId);
		}
...
void Actor_UpdateBgCheckInfo(GlobalContext* globalCtx, Actor* actor, f32 wallCheckHeight, f32 wallCheckRadius, f32 ceilingCheckHeight, s32 flags)
{
	...
	if((actor->floorBgId != BGCHECK_SCENE) && (actor->bgCheckFlags & BG_STATE_0))
	{
	...
	if(flags & 1)
	{
		if((!(flags & 0x80) && BgCheck_EntitySphVsWall3(&globalCtx->colCtx, &sp64, &actor->world.pos, &actor->prevPos, wallCheckRadius, &actor->wallPoly, &bgId, actor, wallCheckHeight)) ||
		   ((flags & 0x80) && BgCheck_EntitySphVsWall4(&globalCtx->colCtx, &sp64, &actor->world.pos, &actor->prevPos, wallCheckRadius, &actor->wallPoly, &bgId, actor, wallCheckHeight)))
		{
			wallPoly = actor->wallPoly;
			Math_Vec3f_Copy(&actor->world.pos, &sp64);
			actor->wallYaw = Math_Atan2S(wallPoly->normal.z, wallPoly->normal.x);
			actor->bgCheckFlags |= BG_STATE_3;
			actor->wallBgId = bgId;
		}
		else
		{
	...
	if(flags & 2)
	{
		...
		if(BgCheck_EntityCheckCeiling(&globalCtx->colCtx, &sp58, &sp64, (ceilingCheckHeight + sp74) - 10.0f, &sCurCeilingPoly, &sCurCeilingBgId, actor))
		{
			actor->bgCheckFlags |= BG_STATE_4;
	...
	if(flags & 4)
	{
		...
		if(WaterBox_GetSurface1(globalCtx, &globalCtx->colCtx, actor->world.pos.x, actor->world.pos.z, &waterBoxYSurface, &waterBox))
		{
			actor->yDistToWater = waterBoxYSurface - actor->world.pos.y;
			if(actor->yDistToWater < 0.0f)
			{
				actor->bgCheckFlags &= ~(BG_STATE_5 | BG_STATE_6);
			}
			else
			{
				if(!(actor->bgCheckFlags & BG_STATE_5))
				{
					actor->bgCheckFlags |= BG_STATE_6;
					if(!(flags & 0x40))
					{
...
Gfx* func_8002E830(Vec3f* object, Vec3f* eye, Vec3f* lightDir, GraphicsContext* gfxCtx, Gfx* gfx, Hilite** hilite)
{
	...
	*hilite = (Hilite*)Graph_Alloc(gfxCtx, sizeof(Hilite));
	...
	if(HREG(80) == 6)
	{
		osSyncPrintf("z_actor.c 3529 eye=[%f(%f) %f %f] object=[%f %f %f] light_direction=[%f %f %f]\n", correctedEyeX, eye->x, eye->y, eye->z, object->x, object->y, object->z, lightDir->x, lightDir->y, lightDir->z);
	}

	func_800ABE74(correctedEyeX, eye->y, eye->z);
	guLookAtHilite(&D_8015BBA8, lookAt, *hilite, correctedEyeX, eye->y, eye->z, object->x, object->y, object->z, 0.0f, 1.0f, 0.0f, lightDir->x, lightDir->y, lightDir->z, lightDir->x, lightDir->y, lightDir->z, 0x10, 0x10);
...
f32 func_8002EFC0(Actor* actor, Player* player, s16 arg2)
{
	...
	if(player->targetedActor != NULL)
	{
		...
		{
			f32 ret = actor->xyzDistToPlayerSq - actor->xyzDistToPlayerSq * 0.8f * ((0x4000 - yawTempAbs) * (1.0f / 0x8000));
...
TargetRangeParams D_80115FF8[] = {
    TARGET_RANGE(70, 140),    TARGET_RANGE(170, 255),	    TARGET_RANGE(280, 5600),	  TARGET_RANGE(350, 525), TARGET_RANGE(700, 1050),
    TARGET_RANGE(1000, 1500), TARGET_RANGE(100, 105.36842), TARGET_RANGE(140, 163.33333), TARGET_RANGE(240, 576), TARGET_RANGE(280, 280000),
};
...
s32 func_8002F1C4(Actor* actor, GlobalContext* globalCtx, f32 arg2, f32 arg3, u32 exchangeItemId)
{
	...
	if((player->actor.flags & ACTOR_FLAG_8) || ((exchangeItemId != EXCH_ITEM_NONE) && Player_InCsMode(globalCtx)) ||
	   (!actor->isTargeted && ((arg3 < fabsf(actor->yDistToPlayer)) || (player->targetActorDistance < actor->xzDistToPlayer) || (arg2 < actor->xzDistToPlayer))))
	{
		return false;
	}
...
void Actor_DrawFaroresWindPointer(GlobalContext* globalCtx)
{
	...
	if(params)
	{
		...
		else if(D_8015BC18 > 0.0f)
		{
			...
			EffectSsKiraKira_SpawnDispersed(globalCtx, &effectPos, &effectVel, &effectAccel, &effectPrimCol, &effectEnvCol, 1000, 16);
		...
		if((globalCtx->csCtx.state == CS_STATE_IDLE) && (((void)0, gSaveContext.respawn[RESPAWN_MODE_TOP].entranceIndex) == ((void)0, gSaveContext.entranceIndex)) &&
		   (((void)0, gSaveContext.respawn[RESPAWN_MODE_TOP].roomIndex) == globalCtx->roomCtx.curRoom.num))
		{
			...
			Matrix_Translate(((void)0, gSaveContext.respawn[RESPAWN_MODE_TOP].pos.x), ((void)0, gSaveContext.respawn[RESPAWN_MODE_TOP].pos.y) + yOffset, ((void)0, gSaveContext.respawn[RESPAWN_MODE_TOP].pos.z), MTXMODE_NEW);
...
u32 D_80116068[] = {
    0x100000C0, 0x100000C0, 0x00000000, 0x100004C0, 0x00000080, 0x300000C0, 0x10000080, 0x00000000, 0x300000C0, 0x100004C0, 0x00000000, 0x100000C0,
};
...
void Actor_UpdateAll(GlobalContext* globalCtx, ActorContext* actorCtx)
{
	...
	if(KREG(0) == -100)
	{
		...
		KREG(0) = 0;
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_CLEAR_TAG, refActor->world.pos.x, refActor->world.pos.y + 100.0f, refActor->world.pos.z, 0, 0, 0, 1);
	}
	...
	for(i = 0; i < ARRAY_COUNT(actorCtx->actorLists); i++, sp80++)
	{
		...
		while(actor != NULL)
		{
			...
			else if((unkFlag && !(actor->flags & unkFlag)) || (!unkFlag && unkCondition && (sp74 != actor) && (actor != player->naviActor) && (actor != player->heldActor) && (&player->actor != actor->parent)))
			{
...
void Actor_Draw(GlobalContext* globalCtx, Actor* actor)
{
	...
	{
		func_800D1694(
		    actor->world.pos.x + globalCtx->mainCamera.skyboxOffset.x, actor->world.pos.y + (f32)((actor->shape.yOffset * actor->scale.y) + globalCtx->mainCamera.skyboxOffset.y), actor->world.pos.z + globalCtx->mainCamera.skyboxOffset.z,
		    &actor->shape.rot);
	}
	else
	{
		func_800D1694(actor->world.pos.x, actor->world.pos.y + (actor->shape.yOffset * actor->scale.y), actor->world.pos.z, &actor->shape.rot);
	}
...
void func_80030FA8(GraphicsContext* gfxCtx)
{
	...
	gDPLoadTextureBlock(POLY_XLU_DISP++, gLensOfTruthMaskTex, G_IM_FMT_I, G_IM_SIZ_8b, 64, 64, 0, G_TX_MIRROR | G_TX_CLAMP, G_TX_MIRROR | G_TX_CLAMP, 6, 6, G_TX_NOLOD, G_TX_NOLOD);
...
void func_8003115C(GlobalContext* globalCtx, s32 numInvisibleActors, Actor** invisibleActors)
{
	...
	{
		gDPSetOtherMode(
		    POLY_XLU_DISP++, G_AD_DISABLE | G_CD_MAGICSQ | G_CK_NONE | G_TC_FILT | G_TF_BILERP | G_TT_NONE | G_TL_TILE | G_TD_CLAMP | G_TP_NONE | G_CYC_1CYCLE | G_PM_NPRIMITIVE, G_AC_THRESHOLD | G_ZS_PRIM | Z_UPD | G_RM_CLD_SURF | G_RM_CLD_SURF2);
	...
	{
		gDPSetOtherMode(
		    POLY_XLU_DISP++, G_AD_DISABLE | G_CD_MAGICSQ | G_CK_NONE | G_TC_FILT | G_TF_BILERP | G_TT_NONE | G_TL_TILE | G_TD_CLAMP | G_TP_NONE | G_CYC_1CYCLE | G_PM_NPRIMITIVE,
		...
		gDPSetCombineLERP(POLY_XLU_DISP++, PRIMITIVE, TEXEL0, PRIM_LOD_FRAC, 0, PRIMITIVE, TEXEL0, PRIM_LOD_FRAC, 0, PRIMITIVE, TEXEL0, PRIM_LOD_FRAC, 0, PRIMITIVE, TEXEL0, PRIM_LOD_FRAC, 0);
	...
	if(globalCtx->roomCtx.curRoom.showInvisActors != 0)
	{
		...
		gDPSetOtherMode(
		    POLY_XLU_DISP++, G_AD_DISABLE | G_CD_MAGICSQ | G_CK_NONE | G_TC_FILT | G_TF_BILERP | G_TT_NONE | G_TL_TILE | G_TD_CLAMP | G_TP_NONE | G_CYC_1CYCLE | G_PM_NPRIMITIVE, G_AC_THRESHOLD | G_ZS_PRIM | G_RM_CLD_SURF | G_RM_CLD_SURF2);
		gDPSetCombineMode(POLY_XLU_DISP++, G_CC_MODULATEIA_PRIM, G_CC_MODULATEIA_PRIM);
		gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, 255, 0, 0, 255);
...
void Draw_Actors(GlobalContext* globalCtx, ActorContext* actorCtx)
{
	...
	for(i = 0; i < ARRAY_COUNT(actorCtx->actorLists); i++, actorListEntry++)
	{
		...
		while(actor != NULL)
		{
			...
			{
				SkinMatrix_Vec3fMtxFMultXYZW(&globalCtx->viewProjectionMtxF, &actor->world.pos, &actor->projectedPos, &actor->projectedW);
			}
			...
			if((HREG(64) != 1) || ((HREG(65) != -1) && (HREG(65) != HREG(66))) || (HREG(71) == 0))
			{
				...
				{
					if((actor->flags & ACTOR_FLAG_7) && ((globalCtx->roomCtx.curRoom.showInvisActors == 0) || (globalCtx->actorCtx.unk_03 != 0) || (actor->room != globalCtx->roomCtx.curRoom.num)))
					{
...
void func_80031B14(GlobalContext* globalCtx, ActorContext* actorCtx)
{
	...
	for(i = 0; i < ARRAY_COUNT(actorCtx->actorLists); i++)
	{
		...
		while(actor != NULL)
		{
			if((actor->room >= 0) && (actor->room != globalCtx->roomCtx.curRoom.num) && (actor->room != globalCtx->roomCtx.prevRoom.num))
			{
				if(!actor->isDrawn)
				{
...
Actor* Actor_RemoveFromCategory(GlobalContext* globalCtx, ActorContext* actorCtx, Actor* actorToRemove)
{
	...
	if((actorToRemove->room == globalCtx->roomCtx.curRoom.num) && (actorToRemove->category == ACTORCAT_ENEMY) && (actorCtx->actorLists[ACTORCAT_ENEMY].length == 0))
	{
...
void Actor_SpawnTransitionActors(GlobalContext* globalCtx, ActorContext* actorCtx)
{
	...
	for(i = 0; i < numActors; i++)
	{
		...
		{
			if(((transitionActor->sides[0].room >= 0) && ((transitionActor->sides[0].room == globalCtx->roomCtx.curRoom.num) || (transitionActor->sides[0].room == globalCtx->roomCtx.prevRoom.num))) ||
			   ((transitionActor->sides[1].room >= 0) && ((transitionActor->sides[1].room == globalCtx->roomCtx.curRoom.num) || (transitionActor->sides[1].room == globalCtx->roomCtx.prevRoom.num))))
			{
				Actor_Spawn(actorCtx, globalCtx, (s16)(transitionActor->id & 0x1FFF), transitionActor->pos.x, transitionActor->pos.y, transitionActor->pos.z, 0, transitionActor->rotY, 0, (i << 0xA) + transitionActor->params);
...
{
	return Actor_Spawn(actorCtx, globalCtx, actorEntry->id, actorEntry->pos.x, actorEntry->pos.y, actorEntry->pos.z, actorEntry->rot.x, actorEntry->rot.y, actorEntry->rot.z, actorEntry->params);
}
		...
		if((actor->update != NULL) && ((Player*)actor != player) && CHECK_FLAG_ALL(actor->flags, ACTOR_FLAG_VISIBLE))
		{
			...
			if(actor != sp84)
			{
				...
				if((var < D_8015BBF0) && func_8002F090(actor, var) && func_80032880(globalCtx, actor) &&
				   (!BgCheck_CameraLineTest1(&globalCtx->colCtx, &player->actor.focus.pos, &actor->focus.pos, &sp70, &sp80, 1, 1, 1, 1, &sp7C) || SurfaceType_IsIgnoredByProjectiles(&globalCtx->colCtx, sp80, sp7C)))
				{
...
u8 D_801160A0[] = {
    ACTORCAT_BOSS, ACTORCAT_ENEMY, ACTORCAT_BG, ACTORCAT_EXPLOSIVE, ACTORCAT_NPC, ACTORCAT_ITEMACTION, ACTORCAT_CHEST, ACTORCAT_SWITCH, ACTORCAT_PROP, ACTORCAT_MISC, ACTORCAT_DOOR, ACTORCAT_SWITCH,
};
...
s32 BodyBreak_SpawnParts(Actor* actor, BodyBreak* bodyBreak, GlobalContext* globalCtx, s16 type)
{
	...
	while(bodyBreak->count > 0)
	{
		...
		spawnedEnPart = (EnPart*)Actor_SpawnAsChild(&globalCtx->actorCtx, actor, globalCtx, ACTOR_EN_PART, mtx->xw, mtx->yw, mtx->zw, 0, 0, objBankIndex, type);
...
Actor* Actor_GetProjectileActor(GlobalContext* globalCtx, Actor* refActor, f32 radius)
{
	...
	while(actor != NULL)
	{
		...
		else
		{
			...
			if((Math_Vec3f_DistXYZ(&refActor->world.pos, &actor->world.pos) > radius) || (((ArmsHook*)actor)->timer == 0))
			{
				actor = actor->next;
			}
			else
			{
				...
				if(CollisionCheck_CylSideVsLineSeg(refActor->colChkInfo.cylRadius, refActor->colChkInfo.cylHeight, 0.0f, &refActor->world.pos, &actor->world.pos, &spA8, &sp90, &sp84))
				{
					return actor;
				}
				else
...
s16 Actor_TestFloorInDirection(Actor* actor, GlobalContext* globalCtx, f32 distance, s16 angle)
{
	...
	ret = actor->bgCheckFlags & BG_STATE_0;
...
static struct_80116130 D_80116130[] = {
    {{0x2AA8, 0xF1C8, 0x18E2, 0x1554, 0x0000, 0x0000, 1}, 170.0f, 0x3FFC}, {{0x2AA8, 0xEAAC, 0x1554, 0x1554, 0xF8E4, 0x0E38, 1}, 170.0f, 0x3FFC}, {{0x31C4, 0xE390, 0x0E38, 0x0E38, 0xF1C8, 0x071C, 1}, 170.0f, 0x3FFC},
    {{0x1554, 0xF1C8, 0x0000, 0x071C, 0xF8E4, 0x0000, 1}, 170.0f, 0x3FFC}, {{0x2AA8, 0xF8E4, 0x071C, 0x0E38, 0xD558, 0x2AA8, 1}, 170.0f, 0x3FFC}, {{0x0000, 0xE390, 0x2AA8, 0x3FFC, 0xF1C8, 0x0E38, 1}, 170.0f, 0x3FFC},
    {{0x2AA8, 0xF1C8, 0x0E38, 0x0E38, 0x0000, 0x0000, 1}, 0.0f, 0x0000},   {{0x2AA8, 0xF1C8, 0x0000, 0x0E38, 0x0000, 0x1C70, 1}, 0.0f, 0x0000},	  {{0x2AA8, 0xF1C8, 0xF1C8, 0x0000, 0x0000, 0x0000, 1}, 0.0f, 0x0000},
    {{0x071C, 0xF1C8, 0x0E38, 0x1C70, 0x0000, 0x0000, 1}, 0.0f, 0x0000},   {{0x0E38, 0xF1C8, 0x0000, 0x1C70, 0x0000, 0x0E38, 1}, 0.0f, 0x0000},	  {{0x2AA8, 0xE390, 0x1C70, 0x0E38, 0xF1C8, 0x0E38, 1}, 0.0f, 0x0000},
...
void func_80034A14(Actor* actor, struct_80034A14_arg1* arg1, s16 arg2, s16 arg3)
{
	...
	func_800344BC(actor, arg1, sp38.unk_00, sp38.unk_04, sp38.unk_02, sp38.unk_06, sp38.unk_0A, sp38.unk_08, sp38.unk_0C);
}
...
Gfx* func_80034B54(GraphicsContext* gfxCtx)
{
	...
	gDPSetRenderMode(displayListHead++, G_RM_FOG_SHADE_A, AA_EN | Z_CMP | Z_UPD | IM_RD | CLR_ON_CVG | CVG_DST_WRAP | ZMODE_XLU | FORCE_BL | GBL_c2(G_BL_CLR_IN, G_BL_A_IN, G_BL_CLR_MEM, G_BL_1MA));
...
void func_80034EC0(SkelAnime* skelAnime, struct_80034EC0_Entry* animations, s32 index)
{
	...
	Animation_Change(skelAnime, animations->animation, animations->playbackSpeed, animations->startFrame, frameCount, animations->mode, animations->transitionRate);
}
...
s32 func_80035124(Actor* actor, GlobalContext* globalCtx)
{
	...
	switch(actor->params)
	{
		case 0:
			...
			else if(!(actor->bgCheckFlags & BG_STATE_0))
			{
				Actor_MoveForward(actor);
				Math_SmoothStepToF(&actor->speedXZ, 0.0f, 1.0f, 0.1f, 0.0f);
			}
			else if((actor->bgCheckFlags & BG_STATE_1) && (actor->velocity.y < -4.0f))
			{
				ret = 1;
			}
			else
	...
	Actor_UpdateBgCheckInfo(globalCtx, actor, actor->colChkInfo.cylHeight, actor->colChkInfo.cylRadius, actor->colChkInfo.cylRadius, 0x1D);
...
Gfx D_80116280[] = {
    gsDPSetRenderMode(G_RM_FOG_SHADE_A, AA_EN | Z_CMP | Z_UPD | IM_RD | CLR_ON_CVG | CVG_DST_WRAP | ZMODE_XLU | FORCE_BL | GBL_c2(G_BL_CLR_IN, G_BL_A_IN, G_BL_CLR_MEM, G_BL_1MA)),
...
template <class T> static inline void _func_80035844(Vec3f* arg0, Vec3f* arg1, T* arg2, s32 arg3)
{
...
Actor* func_800358DC(Actor* actor, Vec3f* spawnPos, VecRot* spawnRot, f32* arg3, s32 timer, s16* unused, GlobalContext* globalCtx, s16 params, s32 arg8)
{
	...
	spawnedEnPart = (EnPart*)Actor_SpawnAsChild(&globalCtx->actorCtx, actor, globalCtx, ACTOR_EN_PART, spawnPos->x, spawnPos->y, spawnPos->z, spawnRot->x, spawnRot->y, actor->objBankIndex, params);
	if(spawnedEnPart != NULL)
	{
		spawnedEnPart->actor.scale = actor->scale;
