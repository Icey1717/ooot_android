...
Struct_8011FAF0 D_8011FAF0[] = {
    {6, 0x00000000}, {5, 0x00020000}, {4, 0x00030000}, {3, 0x00038000}, {2, 0x0003C000}, {1, 0x0003E000}, {0, 0x0003F000}, {0, 0x0003F800},
};
...
void Environment_Init(GlobalContext* globalCtx2, EnvironmentContext* envCtx, s32 unused)
{
	...
	globalCtx->gfxCtx->callback = Environment_GraphCallback;
	globalCtx->gfxCtx->callbackParam = globalCtx;
	...
	envCtx->adjAmbientColor[0] = envCtx->adjAmbientColor[1] = envCtx->adjAmbientColor[2] = envCtx->adjLight1Color[0] = envCtx->adjLight1Color[1] = envCtx->adjLight1Color[2] = envCtx->adjFogColor[0] = envCtx->adjFogColor[1] = envCtx->adjFogColor[2] =
	    envCtx->adjFogNear = envCtx->adjFogFar = 0;
...
void Environment_UpdateSkybox(u8 skyboxId, EnvironmentContext* envCtx, SkyboxContext* skyboxCtx)
{
	...
	if(skyboxId == SKYBOX_CUTSCENE_MAP)
	{
		...
		{
			if(gSaveContext.skyboxTime >= D_8011FC1C[envCtx->unk_17][i].startTime && (gSaveContext.skyboxTime < D_8011FC1C[envCtx->unk_17][i].endTime || D_8011FC1C[envCtx->unk_17][i].endTime == 0xFFFF))
			{
				if(D_8011FC1C[envCtx->unk_17][i].blend)
				{
					envCtx->skyboxBlend = Environment_LerpWeight(D_8011FC1C[envCtx->unk_17][i].endTime, D_8011FC1C[envCtx->unk_17][i].startTime, ((void)0, gSaveContext.skyboxTime)) * 255;
				}
				else
				{
	...
	else if(skyboxId == SKYBOX_NORMAL_SKY && !envCtx->skyboxDisabled)
	{
		for(i = 0; i < ARRAY_COUNT(D_8011FC1C[envCtx->unk_17]); i++)
		{
			if(gSaveContext.skyboxTime >= D_8011FC1C[envCtx->unk_17][i].startTime && (gSaveContext.skyboxTime < D_8011FC1C[envCtx->unk_17][i].endTime || D_8011FC1C[envCtx->unk_17][i].endTime == 0xFFFF))
			{
				...
				{
					skyboxBlend = Environment_LerpWeight(D_8011FC1C[envCtx->unk_17][i].endTime, D_8011FC1C[envCtx->unk_17][i].startTime, ((void)0, gSaveContext.skyboxTime)) * 255;
				}
				else
				{
					skyboxBlend = Environment_LerpWeight(D_8011FC1C[envCtx->unk_17][i].endTime, D_8011FC1C[envCtx->unk_17][i].startTime, ((void)0, gSaveContext.skyboxTime)) * 255;
		...
		if((envCtx->skybox1Index != newSkybox1Index) && (envCtx->skyboxDmaState == SKYBOX_DMA_INACTIVE))
		{
			...
			DmaMgr_SendRequest2(&envCtx->dmaRequest, skyboxCtx->staticSegments[0], gSkyboxFiles[newSkybox1Index].file.vromStart, size, 0, nullptr, NULL, "../z_kankyo.c", 1264);
		...
		if((envCtx->skybox2Index != newSkybox2Index) && (envCtx->skyboxDmaState == SKYBOX_DMA_INACTIVE))
		{
			...
			DmaMgr_SendRequest2(&envCtx->dmaRequest, skyboxCtx->staticSegments[1], gSkyboxFiles[newSkybox2Index].file.vromStart, size, 0, nullptr, NULL, "../z_kankyo.c", 1281);
		...
		if(envCtx->skyboxDmaState == SKYBOX_DMA_FILE1_DONE)
		{
			...
			if((newSkybox1Index & 1) ^ ((newSkybox1Index & 4) >> 2))
			{
				...
				DmaMgr_SendRequest2(&envCtx->dmaRequest, skyboxCtx->palettes, gSkyboxFiles[newSkybox1Index].palette.vromStart, size, 0, nullptr, NULL, "../z_kankyo.c", 1307);
			}
			else
			{
				size = POINTER_SUB2(gSkyboxFiles[newSkybox1Index].palette.vromEnd, gSkyboxFiles[newSkybox1Index].palette.vromStart);
				DmaMgr_SendRequest2(&envCtx->dmaRequest, POINTER_ADD(skyboxCtx->palettes, size), gSkyboxFiles[newSkybox1Index].palette.vromStart, size, 0, nullptr, NULL, "../z_kankyo.c", 1320);
			}
		}
		...
		if(envCtx->skyboxDmaState == SKYBOX_DMA_FILE2_DONE)
		{
			...
			if((newSkybox2Index & 1) ^ ((newSkybox2Index & 4) >> 2))
			{
				...
				DmaMgr_SendRequest2(&envCtx->dmaRequest, skyboxCtx->palettes, gSkyboxFiles[newSkybox2Index].palette.vromStart, size, 0, nullptr, NULL, "../z_kankyo.c", 1342);
			}
			else
			{
				size = POINTER_SUB2(gSkyboxFiles[newSkybox2Index].palette.vromEnd, gSkyboxFiles[newSkybox2Index].palette.vromStart);

				DmaMgr_SendRequest2(&envCtx->dmaRequest, POINTER_ADD(skyboxCtx->palettes, size), gSkyboxFiles[newSkybox2Index].palette.vromStart, size, 0, nullptr, NULL, "../z_kankyo.c", 1355);
			}
		}
...
void Environment_Update(GlobalContext* globalCtx, EnvironmentContext* envCtx, LightContext* lightCtx, PauseContext* pauseCtx, MessageContext* msgCtx, GameOverContext* gameOverCtx, GraphicsContext* gfxCtx)
{
	...
	if(pauseCtx->state == 0)
	{
		...
		if((pauseCtx->state == 0) && (gameOverCtx->state == GAMEOVER_INACTIVE))
		{
			...
			{
				if((envCtx->unk_1A == 0) && !FrameAdvance_IsEnabled(globalCtx) && (globalCtx->transitionMode == 0 || ((void)0, gSaveContext.gameMode) != 0))
				{
		...
		if(((gSaveContext.sceneSetupIndex >= 5 || gTimeIncrement != 0) && gSaveContext.dayTime > gSaveContext.skyboxTime) || (gSaveContext.dayTime < 0xAAB || gTimeIncrement < 0))
		{
		...
		if(SREG(0) != 0 || CREG(2) != 0)
		{
			...
			OPEN_DISPS(globalCtx->gfxCtx, "../z_kankyo.c", 1682);
			...
			CLOSE_DISPS(globalCtx->gfxCtx, "../z_kankyo.c", 1690);
		}

		if((envCtx->unk_BF != 0xFF) && (envCtx->unk_DC != 2) && (envCtx->unk_BD != envCtx->unk_BF) && (envCtx->unk_D8 >= 1.0f) && (envCtx->unk_BF < 0x20))
		{
		...
		if(envCtx->unk_BF != 0xFE)
		{
			if(!envCtx->indoors && (envCtx->unk_BF == 0xFF))
			{
				...
				{
					if((gSaveContext.skyboxTime >= TIME_ENTRY_1F.startTime) && ((gSaveContext.skyboxTime < TIME_ENTRY_1F.endTime) || TIME_ENTRY_1F.endTime == 0xFFFF))
					{
						u8 blend8[2];
						s16 blend16[2];

						sp8C = Environment_LerpWeight(TIME_ENTRY_1F.endTime, TIME_ENTRY_1F.startTime, ((void)0, gSaveContext.skyboxTime));
						...
						{
							// blend ambient color
							blend8[0] = LERP(lightSettingsList[TIME_ENTRY_1F.unk_04].ambientColor[j], lightSettingsList[TIME_ENTRY_1F.unk_05].ambientColor[j], sp8C);
							blend8[1] = LERP(lightSettingsList[TIME_ENTRY_20.unk_04].ambientColor[j], lightSettingsList[TIME_ENTRY_20.unk_05].ambientColor[j], sp8C);
							*(envCtx->lightSettings.ambientColor + j) = LERP(blend8[0], blend8[1], sp88);
						}

						// set light1 direction for the sun
						envCtx->lightSettings.light1Dir[0] = -(Math_SinS(((void)0, gSaveContext.dayTime) - 0x8000) * 120.0f);
						envCtx->lightSettings.light1Dir[1] = Math_CosS(((void)0, gSaveContext.dayTime) - 0x8000) * 120.0f;
						envCtx->lightSettings.light1Dir[2] = (Math_CosS(((void)0, gSaveContext.dayTime) - 0x8000) * 20.0f);
						...
						{
							// blend light1Color
							blend8[0] = LERP(lightSettingsList[TIME_ENTRY_1F.unk_04].light1Color[j], lightSettingsList[TIME_ENTRY_1F.unk_05].light1Color[j], sp8C);
							blend8[1] = LERP(lightSettingsList[TIME_ENTRY_20.unk_04].light1Color[j], lightSettingsList[TIME_ENTRY_20.unk_05].light1Color[j], sp8C);
							*(envCtx->lightSettings.light1Color + j) = LERP(blend8[0], blend8[1], sp88);

							// blend light2Color
							blend8[0] = LERP(lightSettingsList[TIME_ENTRY_1F.unk_04].light2Color[j], lightSettingsList[TIME_ENTRY_1F.unk_05].light2Color[j], sp8C);
							blend8[1] = LERP(lightSettingsList[TIME_ENTRY_20.unk_04].light2Color[j], lightSettingsList[TIME_ENTRY_20.unk_05].light2Color[j], sp8C);
						...
						for(j = 0; j < 3; j++)
						{
							blend8[0] = LERP(lightSettingsList[TIME_ENTRY_1F.unk_04].fogColor[j], lightSettingsList[TIME_ENTRY_1F.unk_05].fogColor[j], sp8C);
							blend8[1] = LERP(lightSettingsList[TIME_ENTRY_20.unk_04].fogColor[j], lightSettingsList[TIME_ENTRY_20.unk_05].fogColor[j], sp8C);
							*(envCtx->lightSettings.fogColor + j) = LERP(blend8[0], blend8[1], sp88);
						}

						blend16[0] = LERP16((lightSettingsList[TIME_ENTRY_1F.unk_04].fogNear & 0x3FF), (lightSettingsList[TIME_ENTRY_1F.unk_05].fogNear & 0x3FF), sp8C);
						blend16[1] = LERP16(lightSettingsList[TIME_ENTRY_20.unk_04].fogNear & 0x3FF, lightSettingsList[TIME_ENTRY_20.unk_05].fogNear & 0x3FF, sp8C);

						envCtx->lightSettings.fogNear = LERP16(blend16[0], blend16[1], sp88);

						blend16[0] = LERP16(lightSettingsList[TIME_ENTRY_1F.unk_04].fogFar, lightSettingsList[TIME_ENTRY_1F.unk_05].fogFar, sp8C);
						blend16[1] = LERP16(lightSettingsList[TIME_ENTRY_20.unk_04].fogFar, lightSettingsList[TIME_ENTRY_20.unk_05].fogFar, sp8C);
						...
						if(TIME_ENTRY_20.unk_05 >= envCtx->numLightSettings)
						{
							...
							osSyncPrintf(VT_COL(RED, WHITE) "\nPalette setting = [%d] Last palette number = [%d]\n" VT_RST, TIME_ENTRY_20.unk_05, envCtx->numLightSettings - 1);
						}
						break;
					}
				}
			...
			else
			{
				...
				else
				{
					...
					for(i = 0; i < 3; i++)
					{
						envCtx->lightSettings.ambientColor[i] = LERP(lightSettingsList[envCtx->unk_BE].ambientColor[i], lightSettingsList[envCtx->unk_BD].ambientColor[i], envCtx->unk_D8);
						envCtx->lightSettings.light1Dir[i] = LERP16(lightSettingsList[envCtx->unk_BE].light1Dir[i], lightSettingsList[envCtx->unk_BD].light1Dir[i], envCtx->unk_D8);
						envCtx->lightSettings.light1Color[i] = LERP(lightSettingsList[envCtx->unk_BE].light1Color[i], lightSettingsList[envCtx->unk_BD].light1Color[i], envCtx->unk_D8);
						envCtx->lightSettings.light2Dir[i] = LERP16(lightSettingsList[envCtx->unk_BE].light2Dir[i], lightSettingsList[envCtx->unk_BD].light2Dir[i], envCtx->unk_D8);
						envCtx->lightSettings.light2Color[i] = LERP(lightSettingsList[envCtx->unk_BE].light2Color[i], lightSettingsList[envCtx->unk_BD].light2Color[i], envCtx->unk_D8);
						envCtx->lightSettings.fogColor[i] = LERP(lightSettingsList[envCtx->unk_BE].fogColor[i], lightSettingsList[envCtx->unk_BD].fogColor[i], envCtx->unk_D8);
					}
					envCtx->lightSettings.fogNear = LERP16(lightSettingsList[envCtx->unk_BE].fogNear & 0x3FF, lightSettingsList[envCtx->unk_BD].fogNear & 0x3FF, envCtx->unk_D8);
					envCtx->lightSettings.fogFar = LERP16(lightSettingsList[envCtx->unk_BE].fogFar, lightSettingsList[envCtx->unk_BD].fogFar, envCtx->unk_D8);
				}
...
void Environment_DrawSunAndMoon(GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_kankyo.c", 2266);

	if(globalCtx->csCtx.state != 0)
	{
		Math_SmoothStepToF(&globalCtx->envCtx.sunPos.x, -(Math_SinS(((void)0, gSaveContext.dayTime) - 0x8000) * 120.0f) * 25.0f, 1.0f, 0.8f, 0.8f);
		Math_SmoothStepToF(&globalCtx->envCtx.sunPos.y, (Math_CosS(((void)0, gSaveContext.dayTime) - 0x8000) * 120.0f) * 25.0f, 1.0f, 0.8f, 0.8f);
		//! @bug This should be z.
		Math_SmoothStepToF(&globalCtx->envCtx.sunPos.y, (Math_CosS(((void)0, gSaveContext.dayTime) - 0x8000) * 20.0f) * 25.0f, 1.0f, 0.8f, 0.8f);
	}
	else
	{
	...
	if(gSaveContext.entranceIndex != 0xCD || ((void)0, gSaveContext.sceneSetupIndex) != 5)
	{
		...
		gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_kankyo.c", 2364), G_MTX_LOAD);
		func_80093AD0(globalCtx->gfxCtx);
		...
		if(alpha > 0.0f)
		{
			gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_kankyo.c", 2406), G_MTX_LOAD);
			func_8009398C(globalCtx->gfxCtx);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_kankyo.c", 2429);
}
...
void Environment_DrawSunLensFlare(GlobalContext* globalCtx, EnvironmentContext* envCtx, View* view, GraphicsContext* gfxCtx, Vec3f pos, s32 unused)
{
	...
	{
		Environment_DrawLensFlare(globalCtx, &globalCtx->envCtx, &globalCtx->view, globalCtx->gfxCtx, pos, 2000, 370, Math_CosS(((void)0, gSaveContext.dayTime) - 0x8000) * 120.0f, 400, 1);
	}
}
...
void Environment_DrawLensFlare(GlobalContext* globalCtx, EnvironmentContext* envCtx, View* view, GraphicsContext* gfxCtx, Vec3f pos, s32 unused, s16 scale, f32 colorIntensity, s16 screenFillAlpha, u8 arg9)
{
	...
	u32 lensFlareTypes[] = {
	    LENS_FLARE_RING, LENS_FLARE_CIRCLE1, LENS_FLARE_CIRCLE1, LENS_FLARE_CIRCLE1, LENS_FLARE_CIRCLE1, LENS_FLARE_CIRCLE1, LENS_FLARE_CIRCLE1, LENS_FLARE_CIRCLE1, LENS_FLARE_CIRCLE1, LENS_FLARE_CIRCLE1,
	};
	...
	else
	{
		if(arg9)
		{
			...
			if(gSunDepthValue != 0xFFFC || screenPos.x < 0.0f || screenPos.y < 0.0f || screenPos.x > SCREEN_WIDTH || screenPos.y > SCREEN_HEIGHT)
			{
				isOffScreen = true;
			}
		}
		...
		for(i = 0; i < ARRAY_COUNT(lensFlareTypes); i++)
		{
			...
			gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, lensFlareColors[i].r, lensFlareColors[i].g, lensFlareColors[i].b, alpha * envCtx->unk_88);
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_kankyo.c", 2662), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gDPSetCombineLERP(POLY_XLU_DISP++, 0, 0, 0, PRIMITIVE, TEXEL0, 0, PRIMITIVE, 0, 0, 0, 0, PRIMITIVE, TEXEL0, 0, PRIMITIVE, 0);
...
void Environment_DrawRain(GlobalContext* globalCtx, View* view, GraphicsContext* gfxCtx)
{
	...
	if(!(globalCtx->cameraPtrs[0]->unk_14C & 0x100) && (globalCtx->envCtx.unk_EE[2] == 0))
	{
		...
		for(i = 0; i < globalCtx->envCtx.unk_EE[1]; i++)
		{
			...
			Matrix_Translate((temp2 - 0.7f) * 100.0f + x50, (temp1 - 0.7f) * 100.0f + y50, (temp3 - 0.7f) * 100.0f + z50, MTXMODE_NEW);
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_kankyo.c", 2887), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		...
		if(player->actor.world.pos.y < view->eye.y)
		{
			...
			for(i = 0; i < globalCtx->envCtx.unk_EE[1]; i++)
			{
				...
				Matrix_Translate(func_800746DC() * 280.0f + x280, player->actor.world.pos.y + 2.0f, func_800746DC() * 280.0f + z280, MTXMODE_NEW);

				if((LINK_IS_ADULT && ((player->actor.world.pos.y + 2.0f - view->eye.y) > -48.0f)) || (!LINK_IS_ADULT && ((player->actor.world.pos.y + 2.0f - view->eye.y) > -30.0f)))
				{
				...
				gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_kankyo.c", 2940), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
{
	if(((globalCtx->skyboxId != SKYBOX_NONE) && (globalCtx->lightCtx.fogNear < 980)) || (globalCtx->skyboxId == SKYBOX_UNSET_1D))
	{
		f32 alpha;

		OPEN_DISPS(globalCtx->gfxCtx, "../z_kankyo.c", 3032);

		func_800938B4(globalCtx->gfxCtx);
		...
		gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, globalCtx->lightCtx.fogColor[0], globalCtx->lightCtx.fogColor[1], globalCtx->lightCtx.fogColor[2], 255.0f * alpha);
		gDPFillRectangle(POLY_OPA_DISP++, 0, 0, SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1);

		CLOSE_DISPS(globalCtx->gfxCtx, "../z_kankyo.c", 3043);
	}

	if(globalCtx->envCtx.customSkyboxFilter)
	{
		OPEN_DISPS(globalCtx->gfxCtx, "../z_kankyo.c", 3048);

		func_800938B4(globalCtx->gfxCtx);
		gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, globalCtx->envCtx.skyboxFilterColor[0], globalCtx->envCtx.skyboxFilterColor[1], globalCtx->envCtx.skyboxFilterColor[2], globalCtx->envCtx.skyboxFilterColor[3]);
		gDPFillRectangle(POLY_OPA_DISP++, 0, 0, SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1);

		CLOSE_DISPS(globalCtx->gfxCtx, "../z_kankyo.c", 3056);
	}
}

void Environment_DrawLightningFlash(GlobalContext* globalCtx, u8 red, u8 green, u8 blue, u8 alpha)
{
	OPEN_DISPS(globalCtx->gfxCtx, "../z_kankyo.c", 3069);

	func_800938B4(globalCtx->gfxCtx);
	gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, red, green, blue, alpha);
	gDPFillRectangle(POLY_OPA_DISP++, 0, 0, SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_kankyo.c", 3079);
}
...
void Environment_UpdateLightningStrike(GlobalContext* globalCtx)
{
		...
		switch(gLightningStrike.state)
		{
			case LIGHTNING_STRIKE_WAIT:
				...
				if(gLightningStrike.delayTimer > 500.0f)
				{
					...
					Environment_AddLightningBolts(globalCtx, (u8)(Rand_ZeroOne() * (ARRAY_COUNT(sLightningBolts) - 0.1f)) + 1);
					sLightningFlashAlpha = 0;
	...
	{
		Environment_DrawLightningFlash(globalCtx, gLightningStrike.flashRed, gLightningStrike.flashGreen, gLightningStrike.flashBlue, sLightningFlashAlpha);
	}
}
...
void Environment_DrawLightning(GlobalContext* globalCtx, s32 unused)
{
	static void* lightningTextures[] = {
	    gEffLightning1Tex, gEffLightning2Tex, gEffLightning3Tex, gEffLightning4Tex, gEffLightning5Tex, gEffLightning6Tex, gEffLightning7Tex, gEffLightning8Tex, NULL,
	};
	s16 i;
	f32 dx;
	f32 dz;
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_kankyo.c", 3253);
	...
	for(i = 0; i < ARRAY_COUNT(sLightningBolts); i++)
	{
		...
		{
			Matrix_Translate(sLightningBolts[i].pos.x + sLightningBolts[i].offset.x, sLightningBolts[i].pos.y + sLightningBolts[i].offset.y, sLightningBolts[i].pos.z + sLightningBolts[i].offset.z, MTXMODE_NEW);
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_kankyo.c", 3333), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_kankyo.c", 3353);
}
	...
	switch(globalCtx->envCtx.unk_E0)
	{
		...
		case 6:
			if((gSaveContext.dayTime < 0xCAAC) && (gSaveContext.dayTime > 0x4555))
			{
				...
				Common_PlaySfx(NA_SE_EV_CHICKEN_CRY_M);
				if((Inventory_ReplaceItem(globalCtx, ITEM_WEIRD_EGG, ITEM_CHICKEN) || Inventory_ReplaceItem(globalCtx, ITEM_POCKET_EGG, ITEM_POCKET_CUCCO)) && globalCtx->csCtx.state == 0 && !Player_InCsMode(globalCtx))
				{
...
void Environment_DrawCustomLensFlare(GlobalContext* globalCtx)
{
	...
	if(gCustomLensFlareOn)
	{
		...
		Environment_DrawLensFlare(globalCtx, &globalCtx->envCtx, &globalCtx->view, globalCtx->gfxCtx, pos, gLensFlareUnused, gLensFlareScale, gLensFlareColorIntensity, gLensFlareScreenFillAlpha, 0);
	}
}
...
void Environment_InitGameOverLights(GlobalContext* globalCtx)
{
	...
	Lights_PointNoGlowSetInfo(&sNGameOverLightInfo, (s16)player->actor.world.pos.x - 10.0f, (s16)player->actor.world.pos.y + 10.0f, (s16)player->actor.world.pos.z - 10.0f, 0, 0, 0, 255);
	sNGameOverLightNode = LightContext_InsertLight(globalCtx, &globalCtx->lightCtx, &sNGameOverLightInfo);

	Lights_PointNoGlowSetInfo(&sSGameOverLightInfo, (s16)player->actor.world.pos.x + 10.0f, (s16)player->actor.world.pos.y + 10.0f, (s16)player->actor.world.pos.z + 10.0f, 0, 0, 0, 255);
...
void Environment_FadeInGameOverLights(GlobalContext* globalCtx)
{
	...
	Lights_PointNoGlowSetInfo(
	    &sNGameOverLightInfo, (s16)player->actor.world.pos.x - 10.0f, (s16)player->actor.world.pos.y + 10.0f, (s16)player->actor.world.pos.z - 10.0f, sGameOverLightsIntensity, sGameOverLightsIntensity, sGameOverLightsIntensity, 255);
	Lights_PointNoGlowSetInfo(
	    &sSGameOverLightInfo, (s16)player->actor.world.pos.x + 10.0f, (s16)player->actor.world.pos.y + 10.0f, (s16)player->actor.world.pos.z + 10.0f, sGameOverLightsIntensity, sGameOverLightsIntensity, sGameOverLightsIntensity, 255);
...
void func_800766C4(GlobalContext* globalCtx)
{
	...
	if(globalCtx->envCtx.unk_EE[1] != max && ((globalCtx->frames % 8) == 0))
	{
...
void Environment_DrawSandstorm(GlobalContext* globalCtx, u8 sandstormState)
{
	...
	gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(globalCtx->gfxCtx, 0, (u32)sp96 % 0x1000, 0, 0x200, 0x20, 1, (u32)sp94 % 0x1000, 0xFFF - ((u32)sp92 % 0x1000), 0x100, 0x40));
