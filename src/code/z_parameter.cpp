...
static s16 sExtraItemBases[] = {
    ITEM_STICK, ITEM_STICK, ITEM_NUT, ITEM_NUT, ITEM_BOMB, ITEM_BOMB, ITEM_BOMB, ITEM_BOMB, ITEM_BOW, ITEM_BOW, ITEM_BOW, ITEM_SEEDS, ITEM_BOMBCHU, ITEM_BOMBCHU, ITEM_STICK, ITEM_STICK, ITEM_NUT, ITEM_NUT,
};
...
static Gfx sSetupDL_80125A60[] = {
    gsDPPipeSync(),
    gsSPClearGeometryMode(G_ZBUFFER | G_SHADE | G_CULL_BOTH | G_FOG | G_LIGHTING | G_TEXTURE_GEN | G_TEXTURE_GEN_LINEAR | G_SHADING_SMOOTH | G_LOD),
...
void Interface_ChangeAlpha(u16 alphaType)
{
	...
	{
		osSyncPrintf("ALPHA-TYPE=%d  LAST_TIME_TYPE=%d\n", alphaType, gSaveContext.unk_13EE);
...
void Interface_SetButtonStatus(GlobalContext* globalCtx)
{
	...
	if((gSaveContext.cutsceneIndex < 0xFFF0) || ((globalCtx->sceneNum == SCENE_SPOT20) && (gSaveContext.cutsceneIndex == 0xFFF0)))
	{
		...
		if((player->stateFlags1 & PLAYER_STATE_HORSE_MOUNTED) || (globalCtx->shootingGalleryStatus > 1) || ((globalCtx->sceneNum == SCENE_BOWLING) && Flags_GetSwitch(globalCtx, 0x38)))
		{
			if(gSaveContext.equips.buttonItems[0] != ITEM_NONE)
			{
				...
				if((gSaveContext.equips.buttonItems[0] != ITEM_SLINGSHOT) && (gSaveContext.equips.buttonItems[0] != ITEM_BOW) && (gSaveContext.equips.buttonItems[0] != ITEM_BOMBCHU) && (gSaveContext.equips.buttonItems[0] != ITEM_NONE))
				{
		...
		else if(msgCtx->msgMode == MSGMODE_NONE)
		{
			if((Player_GetRoomTimer(globalCtx) >= ROOMTIMER_DEEP_UNDERWATER) && (Player_GetRoomTimer(globalCtx) < ROOMTIMER_UNKN5))
			{
				...
				for(i = 1; i < 4; i++)
				{
					if(Player_GetRoomTimer(globalCtx) == ROOMTIMER_DEEP_UNDERWATER)
					{
						...
						if((gSaveContext.equips.buttonItems[i] != ITEM_HOOKSHOT) && (gSaveContext.equips.buttonItems[i] != ITEM_LONGSHOT) && (gSaveContext.equips.buttonItems[i] != ITEM_BOOTS_IRON) &&
					...
					else
					{
						...
						if((gSaveContext.equips.buttonItems[i] != ITEM_BOOTS_IRON) && (gSaveContext.equips.buttonItems[i] != ITEM_BOOTS_HOVER))
							gSaveContext.buttonStatus[i] = BTN_DISABLED;
					}
				}
			...
			else if((gSaveContext.eventInf[0] & 0xF) == 1)
			{
				...
				else
				{
					...
					if((gSaveContext.equips.buttonItems[0] == ITEM_NONE) || (gSaveContext.equips.buttonItems[0] == ITEM_BOW))
					{
						if((gSaveContext.equips.buttonItems[0] != ITEM_SWORD_KOKIRI) && (gSaveContext.equips.buttonItems[0] != ITEM_SWORD_MASTER) && (gSaveContext.equips.buttonItems[0] != ITEM_SWORD_BGS) &&
				...
				for(i = 1; i < 4; i++)
				{
					if((gSaveContext.equips.buttonItems[i] != ITEM_OCARINA_FAIRY) && (gSaveContext.equips.buttonItems[i] != ITEM_OCARINA_TIME))
					{
						if(gSaveContext.buttonStatus[i] == BTN_ENABLED)
						{
							sp28 = 1;
			...
			else
			{
				...
				{
					if((gSaveContext.equips.buttonItems[0] == ITEM_SLINGSHOT) || (gSaveContext.equips.buttonItems[0] == ITEM_BOW) || (gSaveContext.equips.buttonItems[0] == ITEM_BOMBCHU) ||
					   (gSaveContext.equips.buttonItems[0] == ITEM_NONE))
					{
					...
					else if((gSaveContext.buttonStatus[0] & 0xFF) == BTN_DISABLED)
					{
						...
						if(((gSaveContext.buttonStatus[0] & 0xFF) == BTN_DISABLED) || ((gSaveContext.buttonStatus[0] & 0xFF) == BTN_ENABLED))
						{
				...
				if(interfaceCtx->restrictions.bottles != 0)
				{
					for(i = 1; i < 4; i++)
					{
						if((gSaveContext.equips.buttonItems[i] >= ITEM_BOTTLE) && (gSaveContext.equips.buttonItems[i] <= ITEM_POE))
						{
							if(gSaveContext.buttonStatus[i] == BTN_ENABLED)
							{
								sp28 = 1;
				...
				if(interfaceCtx->restrictions.tradeItems != 0)
				{
					for(i = 1; i < 4; i++)
					{
						if((gSaveContext.equips.buttonItems[i] >= ITEM_WEIRD_EGG) && (gSaveContext.equips.buttonItems[i] <= ITEM_CLAIM_CHECK))
						{
							if(gSaveContext.buttonStatus[i] == BTN_ENABLED)
							{
								sp28 = 1;
				...
				if(interfaceCtx->restrictions.dinsNayrus != 0)
				{
					for(i = 1; i < 4; i++)
					{
						if((gSaveContext.equips.buttonItems[i] == ITEM_DINS_FIRE) || (gSaveContext.equips.buttonItems[i] == ITEM_NAYRUS_LOVE))
						{
							if(gSaveContext.buttonStatus[i] == BTN_ENABLED)
							{
								sp28 = 1;
				...
				if(interfaceCtx->restrictions.all != 0)
				{
					...
					{
						if((gSaveContext.equips.buttonItems[i] != ITEM_OCARINA_FAIRY) && (gSaveContext.equips.buttonItems[i] != ITEM_OCARINA_TIME) &&
						   !((gSaveContext.equips.buttonItems[i] >= ITEM_BOTTLE) && (gSaveContext.equips.buttonItems[i] <= ITEM_POE)) &&
						   !((gSaveContext.equips.buttonItems[i] >= ITEM_WEIRD_EGG) && (gSaveContext.equips.buttonItems[i] <= ITEM_CLAIM_CHECK)))
						{
							if((globalCtx->sceneNum != SCENE_TAKARAYA) || (gSaveContext.equips.buttonItems[i] != ITEM_LENS))
							{
								if(gSaveContext.buttonStatus[i] == BTN_ENABLED)
								{
									sp28 = 1;
				...
				else if(interfaceCtx->restrictions.all == 0)
				{
					for(i = 1; i < 4; i++)
					{
						if((gSaveContext.equips.buttonItems[i] != ITEM_DINS_FIRE) && (gSaveContext.equips.buttonItems[i] != ITEM_HOOKSHOT) && (gSaveContext.equips.buttonItems[i] != ITEM_LONGSHOT) &&
						   (gSaveContext.equips.buttonItems[i] != ITEM_FARORES_WIND) && (gSaveContext.equips.buttonItems[i] != ITEM_NAYRUS_LOVE) && (gSaveContext.equips.buttonItems[i] != ITEM_OCARINA_FAIRY) &&
						   (gSaveContext.equips.buttonItems[i] != ITEM_OCARINA_TIME) && !((gSaveContext.equips.buttonItems[i] >= ITEM_BOTTLE) && (gSaveContext.equips.buttonItems[i] <= ITEM_POE)) &&
						   !((gSaveContext.equips.buttonItems[i] >= ITEM_WEIRD_EGG) && (gSaveContext.equips.buttonItems[i] <= ITEM_CLAIM_CHECK)))
						{
							if(gSaveContext.buttonStatus[i] == BTN_DISABLED)
							{
...
void Interface_SetSceneRestrictions(GlobalContext* globalCtx)
{
	...
	osSyncPrintf("Data settings related to button display scene_data_ID=%d\n", globalCtx->sceneNum);
	...
	do
	{
		...
		if(sRestrictionFlags[i].scene == currentScene)
		{
			...
			osSyncPrintf("h_gage=%d, b_button=%d, a_button=%d, c_bottle=%d\n", interfaceCtx->restrictions.hGauge, interfaceCtx->restrictions.bButton, interfaceCtx->restrictions.aButton, interfaceCtx->restrictions.bottles);
			osSyncPrintf("c_warasibe=%d, c_hook=%d, c_ocarina=%d, c_warp=%d\n", interfaceCtx->restrictions.tradeItems, interfaceCtx->restrictions.hookshot, interfaceCtx->restrictions.ocarina, interfaceCtx->restrictions.warpSongs);
			osSyncPrintf("c_sunmoon=%d, m_wind=%d, m_magic=%d, another=%d\n", interfaceCtx->restrictions.sunsSong, interfaceCtx->restrictions.farores, interfaceCtx->restrictions.dinsNayrus, interfaceCtx->restrictions.all);
			osSyncPrintf(VT_RST);
			return;
		}
		i++;
...
Gfx* Gfx_TextureIA8(Gfx* displayListHead, void* texture, s16 textureWidth, s16 textureHeight, s16 rectLeft, s16 rectTop, s16 rectWidth, s16 rectHeight, u16 dsdx, u16 dtdy)
{
	...
	gSPTextureRectangle(displayListHead++, rectLeft * 4, rectTop * 4, (rectLeft + rectWidth) * 4, (rectTop + rectHeight) * 4, G_TX_RENDERTILE, 0, 0, dsdx, dtdy);
...
{
	gDPLoadTextureBlock(displayListHead++, texture, G_IM_FMT_I, G_IM_SIZ_8b, textureWidth, textureHeight, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
		...
		else
		{
			for(i = 0; i < 4; i++)
			{
				   ...
				   ((gSaveContext.equips.buttonItems[i] >= ITEM_WEIRD_EGG) && (gSaveContext.equips.buttonItems[i] <= ITEM_CLAIM_CHECK)))
				{
					...
					gSaveContext.equips.buttonItems[i] = gSaveContext.inventory.items[gSaveContext.equips.cButtonSlots[i - 1]];
				}
			}
...
void Interface_LoadItemIcon1(GlobalContext* globalCtx, u16 button)
{
	...
	{
		case 0:
			interfaceCtx->iconItemSegment1 = (u8*)icon_item_static_lut[gSaveContext.equips.buttonItems[button]];
			break;
		case 1:
			interfaceCtx->iconItemSegment2 = (u8*)icon_item_static_lut[gSaveContext.equips.buttonItems[button]];
			break;
		case 2:
			interfaceCtx->iconItemSegment3 = (u8*)icon_item_static_lut[gSaveContext.equips.buttonItems[button]];
			break;
		case 3:
			interfaceCtx->iconItemSegment4 = (u8*)icon_item_static_lut[gSaveContext.equips.buttonItems[button]];
			break;
		default:
			return;
	}
...
void func_80084BF4(GlobalContext* globalCtx, u16 flag)
{
	if(flag)
	{
		if((gSaveContext.equips.buttonItems[0] == ITEM_SLINGSHOT) || (gSaveContext.equips.buttonItems[0] == ITEM_BOW) || (gSaveContext.equips.buttonItems[0] == ITEM_BOMBCHU) || (gSaveContext.equips.buttonItems[0] == ITEM_FISHING_POLE) ||
		   (gSaveContext.buttonStatus[0] == BTN_DISABLED))
		{
			if((gSaveContext.equips.buttonItems[0] == ITEM_SLINGSHOT) || (gSaveContext.equips.buttonItems[0] == ITEM_BOW) || (gSaveContext.equips.buttonItems[0] == ITEM_BOMBCHU) || (gSaveContext.equips.buttonItems[0] == ITEM_FISHING_POLE))
			{
...
static ItemID sEquipItemMap[4][3] = {
    {ITEM_SWORD_KOKIRI, ITEM_SWORD_MASTER, ITEM_SWORD_BGS}, {ITEM_SHIELD_DEKU, ITEM_SHIELD_HYLIAN, ITEM_SHIELD_MIRROR}, {ITEM_TUNIC_KOKIRI, ITEM_TUNIC_GORON, ITEM_TUNIC_ZORA}, {ITEM_BOOTS_KOKIRI, ITEM_BOOTS_IRON, ITEM_BOOTS_HOVER}};
...
u8 Item_Give(GlobalContext* globalCtx, u8 item)
{
	...
	else if((item >= ITEM_SONG_MINUET) && (item <= ITEM_SONG_STORMS))
	{
		...
		osSyncPrintf("Musical scores = %x (%x) (%x)\n", gSaveContext.inventory.questItems, gBitFlags[item - ITEM_SONG_MINUET + QUEST_SONG_MINUET], gBitFlags[item - ITEM_SONG_MINUET]);
		osSyncPrintf(VT_RST);
	...
	else if(item == ITEM_BOMB)
	{
		...
		osSyncPrintf(" Bomb  Bomb  Bomb  Bomb Bomb   Bomb Bomb \n");
	...
	else if((item >= ITEM_ARROWS_SMALL) && (item <= ITEM_ARROWS_LARGE))
	{
		...
		osSyncPrintf("%dAmmo  Item_MaxGet=%d\n", AMMO(ITEM_BOW), CUR_CAPACITY(UPG_QUIVER));
		...
		if((item != ITEM_MILK_BOTTLE) && (item != ITEM_LETTER_RUTO))
		{
			...
			for(i = 0; i < 4; i++)
			{
				if(gSaveContext.inventory.items[temp + i] == ITEM_BOTTLE)
				{
					...
					osSyncPrintf(
					    "Item_Pt(1)=%d Item_Pt(2)=%d Item_Pt(3)=%d   Empty Bottle=%d   Content=%d\n", gSaveContext.equips.cButtonSlots[0], gSaveContext.equips.cButtonSlots[1], gSaveContext.equips.cButtonSlots[2], temp + i, item);
...
u8 Item_CheckObtainability(u8 item)
{
	...
	else if((item >= ITEM_SWORD_KOKIRI) && (item <= ITEM_SWORD_BGS))
	{
		...
		else if((gBitFlags[item - ITEM_SWORD_KOKIRI] << gEquipShifts[EQUIP_SWORD]) & gSaveContext.inventory.equipment)
		{
			return item;
		}
		else
	...
	else if((item == ITEM_MAGIC_SMALL) || (item == ITEM_MAGIC_LARGE))
	{
		...
		osSyncPrintf("Magic Pot Get_Inf_Table( 25, 0x0100)=%d\n", gSaveContext.infTable[25] & 0x100);
		if(!(gSaveContext.infTable[25] & 0x100))
		{
			return ITEM_NONE;
		}
...
{
	osSyncPrintf("item_no=%x,  c_no=%x,  Pt=%x  Item_Register=%x\n", item, button, gSaveContext.equips.cButtonSlots[button - 1], gSaveContext.inventory.items[gSaveContext.equips.cButtonSlots[button - 1]]);

	// Special case to only empty half of a Lon Lon Milk Bottle
	if((gSaveContext.inventory.items[gSaveContext.equips.cButtonSlots[button - 1]] == ITEM_MILK_BOTTLE) && (item == ITEM_BOTTLE))
	{
		item = ITEM_MILK_HALF;
	}
	...
	KaleidoScope_FindPage(PAUSE_ITEM)->cursorItem() = item;
...
void Interface_LoadActionLabel(InterfaceContext* interfaceCtx, u16 action, s16 loadOffset)
{
	...
	if((action != DO_ACTION_NONE) && (action != DO_ACTION_MAX + DO_ACTION_NONE) && (action != 2 * DO_ACTION_MAX + DO_ACTION_NONE))
	{
		...
		{
			case 0:
				interfaceCtx->doActionSegment1 = (u8*)do_action_static_lut[action];
				break;
			case 1:
				interfaceCtx->doActionSegment2 = (u8*)do_action_static_lut[action];
				break;
			default:
				break;
		}
...
void Interface_SetNaviCall(GlobalContext* globalCtx, u16 naviCallState)
{
	...
	if(((naviCallState == 0x1D) || (naviCallState == 0x1E)) && !interfaceCtx->naviCalling && (globalCtx->csCtx.state == CS_STATE_IDLE))
	{
		// clang-format off
...
s32 Health_ChangeBy(GlobalContext* globalCtx, s16 healthChange)
{
	...
	osSyncPrintf("＊＊＊＊＊  Fluctuation=%d (now=%d, max=%d)  ＊＊＊", healthChange, gSaveContext.health, gSaveContext.healthCapacity);
...
{
	// "Item = (%d)    Amount = (%d + %d)"
	osSyncPrintf("Item = (%d)    Amount = (%d + %d)  ", item, AMMO(item), ammoChange);
...
void Interface_UpdateMagicBar(GlobalContext* globalCtx)
{
	...
	switch(gSaveContext.unk_13F0)
	{
		...
		case 7:
			if((globalCtx->pauseCtx.state == 0) && (globalCtx->pauseCtx.debugState == 0) && (msgCtx->msgMode == MSGMODE_NONE) && (globalCtx->gameOverCtx.state == GAMEOVER_INACTIVE) && (globalCtx->sceneLoadFlag == 0) &&
			   (globalCtx->transitionMode == 0) && !Gameplay_InCsMode(globalCtx))
			{
				if((gSaveContext.magic == 0) || ((Player_GetRoomTimer(globalCtx) >= ROOMTIMER_DEEP_UNDERWATER) && (Player_GetRoomTimer(globalCtx) < ROOMTIMER_UNKN5)) ||
				   ((gSaveContext.equips.buttonItems[1] != ITEM_LENS) && (gSaveContext.equips.buttonItems[2] != ITEM_LENS) && (gSaveContext.equips.buttonItems[3] != ITEM_LENS)) || (globalCtx->actorCtx.unk_03 == 0))
				{
					globalCtx->actorCtx.unk_03 = 0;
					Audio_PlaySoundGeneral(NA_SE_SY_GLASSMODE_OFF, &gAudioDefaultPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
					gSaveContext.unk_13F0 = 0;
					sMagicBorderR = sMagicBorderG = sMagicBorderB = 255;
					break;
				}
...
void Interface_DrawMagicBar(GlobalContext* globalCtx)
{
	...
	if(gSaveContext.magicLevel != 0)
	{
		...
		OVERLAY_DISP = Gfx_TextureIA8(OVERLAY_DISP, gMagicBarEndTex, 8, 16, GFX_ALIGN_LEFT(R_MAGIC_BAR_X), magicBarY, 8, 16, 1 << 10, 1 << 10);

		OVERLAY_DISP = Gfx_TextureIA8(OVERLAY_DISP, gMagicBarMidTex, 24, 16, GFX_ALIGN_LEFT(R_MAGIC_BAR_X) + 8, magicBarY, gSaveContext.unk_13F4, 16, 1 << 10, 1 << 10);

		gDPLoadTextureBlock(OVERLAY_DISP++, gMagicBarEndTex, G_IM_FMT_IA, G_IM_SIZ_8b, 8, 16, 0, G_TX_MIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, 3, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);

		gSPTextureRectangle(
		    OVERLAY_DISP++, ((GFX_ALIGN_LEFT(R_MAGIC_BAR_X) + gSaveContext.unk_13F4) + 8) << 2, magicBarY << 2, ((GFX_ALIGN_LEFT(R_MAGIC_BAR_X) + gSaveContext.unk_13F4) + 16) << 2, (magicBarY + 16) << 2, G_TX_RENDERTILE, 256, 0, 1 << 10, 1 << 10);
		...
		if(gSaveContext.unk_13F0 == 4)
		{
			...
			gDPLoadMultiBlock_4b(OVERLAY_DISP++, gMagicBarFillTex, 0, G_TX_RENDERTILE, G_IM_FMT_I, 16, 16, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);

			gSPTextureRectangle(OVERLAY_DISP++, GFX_ALIGN_LEFT(R_MAGIC_FILL_X) << 2, (magicBarY + 3) << 2, (GFX_ALIGN_LEFT(R_MAGIC_FILL_X) + gSaveContext.magic) << 2, (magicBarY + 10) << 2, G_TX_RENDERTILE, 0, 0, 1 << 10, 1 << 10);
			...
			gSPTextureRectangle(OVERLAY_DISP++, GFX_ALIGN_LEFT(R_MAGIC_FILL_X) << 2, (magicBarY + 3) << 2, (GFX_ALIGN_LEFT(R_MAGIC_FILL_X) + gSaveContext.unk_13F8) << 2, (magicBarY + 10) << 2, G_TX_RENDERTILE, 0, 0, 1 << 10, 1 << 10);
		}
		else
		{
...
void Interface_DrawActionLabel(GraphicsContext* gfxCtx, void* texture)
{
	...
	gDPLoadTextureBlock_4b(OVERLAY_DISP++, texture, G_IM_FMT_IA, DO_ACTION_TEX_WIDTH, DO_ACTION_TEX_HEIGHT, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
...
void Interface_DrawItemButtons(GlobalContext* globalCtx)
{
	...
	gDPSetEnvColor(OVERLAY_DISP++, 0, 0, 0, 255);
	OVERLAY_DISP = Gfx_TextureIA8(OVERLAY_DISP, gButtonBackgroundTex, 32, 32, GFX_ALIGN_RIGHT(R_ITEM_BTN_X(0)), R_ITEM_BTN_Y(0), R_ITEM_BTN_WIDTH(0), R_ITEM_BTN_WIDTH(0), R_ITEM_BTN_DD(0) << 1, R_ITEM_BTN_DD(0) << 1);

	// C-Left Button Color & Texture
	gDPPipeSync(OVERLAY_DISP++);
	gDPSetPrimColor(OVERLAY_DISP++, 0, 0, R_C_BTN_COLOR(0), R_C_BTN_COLOR(1), R_C_BTN_COLOR(2), interfaceCtx->cLeftAlpha);
	gSPTextureRectangle(
	    OVERLAY_DISP++, GFX_ALIGN_RIGHT(R_ITEM_BTN_X(1)) << 2, R_ITEM_BTN_Y(1) << 2, (GFX_ALIGN_RIGHT(R_ITEM_BTN_X(1)) + R_ITEM_BTN_WIDTH(1)) << 2, (R_ITEM_BTN_Y(1) + R_ITEM_BTN_WIDTH(1)) << 2, G_TX_RENDERTILE, 0, 0, R_ITEM_BTN_DD(1) << 1,
	    R_ITEM_BTN_DD(1) << 1);

	// C-Down Button Color & Texture
	gDPSetPrimColor(OVERLAY_DISP++, 0, 0, R_C_BTN_COLOR(0), R_C_BTN_COLOR(1), R_C_BTN_COLOR(2), interfaceCtx->cDownAlpha);
	gSPTextureRectangle(
	    OVERLAY_DISP++, GFX_ALIGN_RIGHT(R_ITEM_BTN_X(2)) << 2, R_ITEM_BTN_Y(2) << 2, (GFX_ALIGN_RIGHT(R_ITEM_BTN_X(2)) + R_ITEM_BTN_WIDTH(2)) << 2, (R_ITEM_BTN_Y(2) + R_ITEM_BTN_WIDTH(2)) << 2, G_TX_RENDERTILE, 0, 0, R_ITEM_BTN_DD(2) << 1,
	    R_ITEM_BTN_DD(2) << 1);

	// C-Right Button Color & Texture
	gDPSetPrimColor(OVERLAY_DISP++, 0, 0, R_C_BTN_COLOR(0), R_C_BTN_COLOR(1), R_C_BTN_COLOR(2), interfaceCtx->cRightAlpha);
	gSPTextureRectangle(
	    OVERLAY_DISP++, GFX_ALIGN_RIGHT(R_ITEM_BTN_X(3)) << 2, R_ITEM_BTN_Y(3) << 2, (GFX_ALIGN_RIGHT(R_ITEM_BTN_X(3)) + R_ITEM_BTN_WIDTH(3)) << 2, (R_ITEM_BTN_Y(3) + R_ITEM_BTN_WIDTH(3)) << 2, G_TX_RENDERTILE, 0, 0, R_ITEM_BTN_DD(3) << 1,
	    R_ITEM_BTN_DD(3) << 1);
	...
	if(interfaceCtx->naviCalling && (globalCtx->pauseCtx.state == 0) && (globalCtx->pauseCtx.debugState == 0) && (globalCtx->csCtx.state == CS_STATE_IDLE))
	{
		if(!sCUpInvisible)
		{
			...
			gDPSetCombineMode(OVERLAY_DISP++, G_CC_MODULATEIA_PRIM, G_CC_MODULATEIA_PRIM);
			gSPTextureRectangle(OVERLAY_DISP++, GFX_ALIGN_RIGHT(R_C_UP_BTN_X) << 2, R_C_UP_BTN_Y << 2, (GFX_ALIGN_RIGHT(R_C_UP_BTN_X) + 16) << 2, (R_C_UP_BTN_Y + 16) << 2, G_TX_RENDERTILE, 0, 0, 2 << 10, 2 << 10);
			gDPPipeSync(OVERLAY_DISP++);
	...
	for(temp = 1; temp < 4; temp++)
	{
		if(gSaveContext.equips.buttonItems[temp] > 0xF0)
		{
			...
			OVERLAY_DISP = Gfx_TextureIA8(
			    OVERLAY_DISP, ((u8*)gButtonBackgroundTex + ((32 * 32) * (temp + 1))), 32, 32, GFX_ALIGN_RIGHT(R_ITEM_BTN_X(temp)), R_ITEM_BTN_Y(temp), R_ITEM_BTN_WIDTH(temp), R_ITEM_BTN_WIDTH(temp), R_ITEM_BTN_DD(temp) << 1,
...
void Interface_DrawAmmoCount(GlobalContext* globalCtx, s16 button, s16 alpha)
{
	...
	if((i == ITEM_STICK) || (i == ITEM_NUT) || (i == ITEM_BOMB) || (i == ITEM_BOW) || ((i >= ITEM_BOW_ARROW_FIRE) && (i <= ITEM_BOW_ARROW_LIGHT)) || (i == ITEM_SLINGSHOT) || (i == ITEM_BOMBCHU) || (i == ITEM_BEAN))
	{
		...
		else if((button == 0) && (globalCtx->sceneNum == SCENE_BOWLING) && Flags_GetSwitch(globalCtx, 0x38))
		{
		    ...
		    ((i == ITEM_BOW) && (AMMO(i) == CUR_CAPACITY(UPG_QUIVER))) || ((i == ITEM_BOMB) && (AMMO(i) == CUR_CAPACITY(UPG_BOMB_BAG))) || ((i == ITEM_SLINGSHOT) && (AMMO(i) == CUR_CAPACITY(UPG_BULLET_BAG))) ||
		    ((i == ITEM_STICK) && (AMMO(i) == CUR_CAPACITY(UPG_STICKS))) || ((i == ITEM_NUT) && (AMMO(i) == CUR_CAPACITY(UPG_NUTS))) || ((i == ITEM_BOMBCHU) && (ammo == 50)) || ((i == ITEM_BEAN) && (ammo == 15)))
		{
		...
		if(i != 0)
		{
			OVERLAY_DISP = Gfx_TextureIA8(OVERLAY_DISP, ((u8*)gAmmoDigit0Tex + ((8 * 8) * i)), 8, 8, GFX_ALIGN_RIGHT(R_ITEM_AMMO_X(button)), R_ITEM_AMMO_Y(button), 8, 8, 1 << 10, 1 << 10);
		}
...
void Interface_Draw(GlobalContext* globalCtx)
{
	...
	gSPSegment(OVERLAY_DISP++, 0x08, interfaceCtx->iconItemSegment1);
	gSPSegment(OVERLAY_DISP++, 0x0B, interfaceCtx->mapSegment1);
	...
	if(pauseCtx->debugState == 0)
	{
		...
		OVERLAY_DISP = Gfx_TextureIA8(OVERLAY_DISP, gRupeeCounterIconTex, 16, 16, GFX_ALIGN_LEFT(26), 206, 16, 16, 1 << 10, 1 << 10);
		...
		switch(globalCtx->sceneNum)
		{
			...
			case SCENE_TAKARAYA:
				if(gSaveContext.inventory.dungeonKeys[gSaveContext.mapIndex] >= 0)
				{
					...
					OVERLAY_DISP = Gfx_TextureIA8(OVERLAY_DISP, gSmallKeyCounterIconTex, 16, 16, GFX_ALIGN_LEFT(26), 190, 16, 16, 1 << 10, 1 << 10);
					...
					svar3 = GFX_ALIGN_LEFT(42);

					if(interfaceCtx->counterDigits[2] != 0)
					{
						OVERLAY_DISP = Gfx_TextureI8(OVERLAY_DISP, ((u8*)gCounterDigit0Tex + (8 * 16 * interfaceCtx->counterDigits[2])), 8, 16, svar3, 190, 8, 16, 1 << 10, 1 << 10);
						svar3 += 8;
					}

					OVERLAY_DISP = Gfx_TextureI8(OVERLAY_DISP, ((u8*)gCounterDigit0Tex + (8 * 16 * interfaceCtx->counterDigits[3])), 8, 16, svar3, 190, 8, 16, 1 << 10, 1 << 10);
				}
				break;
			default:
				break;
		...
		{
			OVERLAY_DISP = Gfx_TextureI8(OVERLAY_DISP, ((u8*)gCounterDigit0Tex + (8 * 16 * interfaceCtx->counterDigits[svar2])), 8, 16, GFX_ALIGN_LEFT(svar3), 206, 8, 16, 1 << 10, 1 << 10);
		}
		...
		if(gSaveContext.equips.buttonItems[1] < 0xF0)
		{
			...
			gDPSetCombineMode(OVERLAY_DISP++, G_CC_MODULATERGBA_PRIM, G_CC_MODULATERGBA_PRIM);
			Interface_DrawItemIconTexture(globalCtx, interfaceCtx->iconItemSegment2, 1);
			gDPPipeSync(OVERLAY_DISP++);
			gDPSetCombineLERP(OVERLAY_DISP++, PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, PRIMITIVE, 0, PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, PRIMITIVE, 0);
		...
		if(gSaveContext.equips.buttonItems[2] < 0xF0)
		{
			...
			gDPSetCombineMode(OVERLAY_DISP++, G_CC_MODULATERGBA_PRIM, G_CC_MODULATERGBA_PRIM);
			Interface_DrawItemIconTexture(globalCtx, interfaceCtx->iconItemSegment3, 2);
			gDPPipeSync(OVERLAY_DISP++);
			gDPSetCombineLERP(OVERLAY_DISP++, PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, PRIMITIVE, 0, PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, PRIMITIVE, 0);
		...
		if(gSaveContext.equips.buttonItems[3] < 0xF0)
		{
			...
			gDPSetCombineMode(OVERLAY_DISP++, G_CC_MODULATERGBA_PRIM, G_CC_MODULATERGBA_PRIM);
			Interface_DrawItemIconTexture(globalCtx, interfaceCtx->iconItemSegment4, 3);
			gDPPipeSync(OVERLAY_DISP++);
			gDPSetCombineLERP(OVERLAY_DISP++, PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, PRIMITIVE, 0, PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, PRIMITIVE, 0);
		...
		gDPPipeSync(OVERLAY_DISP++);
		func_8008A8B8(globalCtx, R_A_ICON_Y, R_A_ICON_Y + 45, GFX_ALIGN_RIGHT(R_A_ICON_X), GFX_ALIGN_RIGHT(R_A_ICON_X) + 45);
		...
		if((pauseCtx->state == 6) && (pauseCtx->unk_1E4 == 3))
		{
			...
			if(pauseCtx->equipTargetItem < 0xBF)
			{
				...
				gDPLoadTextureBlock(
				    OVERLAY_DISP++, gItemIconsCurrent[pauseCtx->equipTargetItem], G_IM_FMT_RGBA, G_IM_SIZ_32b, 32, 32, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
			}
			else
			{
				// Magic Arrow Equip Effect
				svar1 = pauseCtx->equipTargetItem - 0xBF;
				gDPSetPrimColor(OVERLAY_DISP++, 0, 0, magicArrowEffectsR[svar1], magicArrowEffectsG[svar1], magicArrowEffectsB[svar1], pauseCtx->equipAnimAlpha);

				if((pauseCtx->equipAnimAlpha > 0) && (pauseCtx->equipAnimAlpha < 255))
				{
					svar1 = (pauseCtx->equipAnimAlpha / 8) / 2;
					pauseCtx->cursorVtx[16].v.ob[0] = pauseCtx->cursorVtx[18].v.ob[0] = svar2 = pauseCtx->cursorVtx[16].v.ob[0] - svar1;
					pauseCtx->cursorVtx[17].v.ob[0] = pauseCtx->cursorVtx[19].v.ob[0] = svar2 = pauseCtx->cursorVtx[16].v.ob[0] + svar1 * 2 + 32;
					pauseCtx->cursorVtx[16].v.ob[1] = pauseCtx->cursorVtx[17].v.ob[1] = svar2 = pauseCtx->cursorVtx[16].v.ob[1] + svar1;
					pauseCtx->cursorVtx[18].v.ob[1] = pauseCtx->cursorVtx[19].v.ob[1] = svar2 = pauseCtx->cursorVtx[16].v.ob[1] - svar1 * 2 - 32;
				}

				gSPVertex(OVERLAY_DISP++, &pauseCtx->cursorVtx[16], 4, 0);
				gDPLoadTextureBlock(OVERLAY_DISP++, gMagicArrowEquipEffectTex, G_IM_FMT_IA, G_IM_SIZ_8b, 32, 32, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
			}
		...
		if((globalCtx->pauseCtx.state == 0) && (globalCtx->pauseCtx.debugState == 0))
		{
			if(gSaveContext.minigameState != 1)
			{
				...
				{
					// Load Carrot Icon
					gDPLoadTextureBlock(OVERLAY_DISP++, gCarrotIconTex, G_IM_FMT_RGBA, G_IM_SIZ_32b, 16, 16, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);

					// Draw 6 carrots
					for(svar1 = 1, svar5 = ZREG(14); svar1 < 7; svar1++, svar5 += 16)
					{ // this took me three hours to figure out please dont laugh
						...
						gSPTextureRectangle(OVERLAY_DISP++, svar5 << 2, ZREG(15) << 2, (svar5 + 16) << 2, (ZREG(15) + 16) << 2, G_TX_RENDERTILE, 0, 0, 1 << 10, 1 << 10);
					}
				}
			}
			else
			{
				...
				svar5 = GFX_ALIGN_LEFT(WREG(32));
				gDPSetPrimColor(OVERLAY_DISP++, 0, 0, 255, 255, 255, interfaceCtx->bAlpha);

				// Target Icon
				gDPLoadTextureBlock(OVERLAY_DISP++, gArcheryScoreIconTex, G_IM_FMT_RGBA, G_IM_SIZ_16b, 24, 16, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);

				gSPTextureRectangle(OVERLAY_DISP++, (svar5 + 28) << 2, ZREG(15) << 2, (svar5 + 52) << 2, (ZREG(15) + 16) << 2, G_TX_RENDERTILE, 0, 0, 1 << 10, 1 << 10);

				// Score Counter
				gDPPipeSync(OVERLAY_DISP++);
				gDPSetCombineLERP(OVERLAY_DISP++, 0, 0, 0, PRIMITIVE, TEXEL0, 0, PRIMITIVE, 0, 0, 0, 0, PRIMITIVE, TEXEL0, 0, PRIMITIVE, 0);

				svar5 = GFX_ALIGN_LEFT(WREG(32) + 6 * 9);

				for(svar1 = svar2 = 0; svar1 < 4; svar1++)
				{
					if(sHBAScoreDigits[svar1] != 0 || (svar2 != 0) || (svar1 >= 3))
					{
						OVERLAY_DISP = Gfx_TextureI8(OVERLAY_DISP, ((u8*)gCounterDigit0Tex + (8 * 16 * sHBAScoreDigits[svar1])), 8, 16, svar5, (ZREG(15) - 2), digitWidth[0], VREG(42), VREG(43) << 1, VREG(43) << 1);
						svar5 += 9;
						svar2++;
					}
				}
		   ...
		   !((globalCtx->sceneNum == SCENE_BOWLING) && Flags_GetSwitch(globalCtx, 0x38)))
		{
			...
			if(((gSaveContext.timer1State != 0) && (gSaveContext.timer1State != 10)) || (gSaveContext.timer2State != 0))
			{
				...
				gDPSetEnvColor(OVERLAY_DISP++, 0, 0, 0, 0);
				svar5 = GFX_ALIGN_GUESS(gSaveContext.timerX[svar6]);
				svar2 = gSaveContext.timerY[svar6];

				OVERLAY_DISP = Gfx_TextureIA8(OVERLAY_DISP, gClockIconTex, 16, 16, svar5, svar2 + 2, 16, 16, 1 << 10, 1 << 10);
...
void Interface_Update(GlobalContext* globalCtx)
{
	...
	if((globalCtx->pauseCtx.state == 0) && (globalCtx->pauseCtx.debugState == 0))
	{
		if((gSaveContext.minigameState == 1) || (gSaveContext.sceneSetupIndex < 4) || ((globalCtx->sceneNum == SCENE_SPOT20) && (gSaveContext.sceneSetupIndex == 4)))
		{
			if((msgCtx->msgMode == MSGMODE_NONE) || ((msgCtx->msgMode != MSGMODE_NONE) && (globalCtx->sceneNum == SCENE_BOWLING)))
			{
	...
	if((globalCtx->pauseCtx.state == 0) && (globalCtx->pauseCtx.debugState == 0) && (msgCtx->msgMode == MSGMODE_NONE) && (globalCtx->sceneLoadFlag == 0) && (globalCtx->gameOverCtx.state == GAMEOVER_INACTIVE) && (globalCtx->transitionMode == 0) &&
