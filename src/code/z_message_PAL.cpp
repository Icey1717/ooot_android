...
void Message_HandleChoiceSelection(GlobalContext* globalCtx, u8 numChoices)
{
	...
	msgCtx->textPosY = (numChoices == 1) ? R_TEXT_CHOICE_YPOS(msgCtx->choiceIndex + 1) : R_TEXT_CHOICE_YPOS(msgCtx->choiceIndex);
}
...
void Message_DrawTextChar(GlobalContext* globalCtx, void* textureImage, Gfx** p)
{
	...
	gDPLoadTextureBlock_4b(gfx++, textureImage, G_IM_FMT_I, FONT_CHAR_TEX_WIDTH, FONT_CHAR_TEX_HEIGHT, 0, G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
	...
	gSPTextureRectangle(gfx++, x << 2, y << 2, (x + sCharTexSize) << 2, (y + sCharTexSize) << 2, G_TX_RENDERTILE, 0, 0, sCharTexScale, sCharTexScale);
	*p = gfx;
}
...
void Message_DrawTextboxIcon(GlobalContext* globalCtx, Gfx** p, s16 x, s16 y)
{
	...
	gDPLoadTextureBlock_4b(gfx++, iconTexture, G_IM_FMT_I, FONT_CHAR_TEX_WIDTH, FONT_CHAR_TEX_HEIGHT, 0, G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
...
u16 Message_DrawItemIcon(GlobalContext* globalCtx, u16 itemId, Gfx** p, u16 i)
{
	...
	{
		gDPLoadTextureBlock(gfx++, (uintptr_t)msgCtx->textboxSegment + MESSAGE_STATIC_TEX_SIZE, G_IM_FMT_RGBA, G_IM_SIZ_32b, 24, 24, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
	}
	else
	{
...
void Message_HandleOcarina(GlobalContext* globalCtx)
{
	...
	if(globalCtx->msgCtx.msgMode == MSGMODE_TEXT_DISPLAYING)
	{
		...
		else
		{
			...
			if(msgCtx->ocarinaAction == OCARINA_ACTION_UNK_0 || msgCtx->ocarinaAction == OCARINA_ACTION_FREE_PLAY || msgCtx->ocarinaAction >= OCARINA_ACTION_CHECK_SARIA)
			{
				msgCtx->msgMode = MSGMODE_OCARINA_STARTING;
				osSyncPrintf("000000000000  -> ");
			}
			else if(msgCtx->ocarinaAction >= OCARINA_ACTION_TEACH_MINUET && msgCtx->ocarinaAction <= OCARINA_ACTION_TEACH_STORMS)
			{
	...
	for(i = 0; i < msgCtx->textDrawPos; i++)
	{
		...
		switch(character)
		{
			...
			case MESSAGE_QUICKTEXT_ENABLE:
				if(i + 1 == msgCtx->textDrawPos && (msgCtx->msgMode == MSGMODE_TEXT_DISPLAYING || (msgCtx->msgMode >= MSGMODE_OCARINA_STARTING && msgCtx->msgMode < MSGMODE_SCARECROW_LONG_RECORDING_START)))
				{
					j = i;
					while(true)
					{
			...
			case MESSAGE_SFX:
				if(msgCtx->msgMode == MSGMODE_TEXT_DISPLAYING && !sMessageHasSetSfx)
				{
					...
					Audio_PlaySoundGeneral(sfxHi | msgCtx->msgBufDecoded[i + 2], &gAudioDefaultPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
				}
				i += 2;
				break;
...
void Message_OpenText(GlobalContext* globalCtx, u16 textId)
{
	...
	if(textId >= 0x0500 && textId < 0x0600)
	{ // text ids 0500 to 0600 are reserved for credits
		sTextIsCredits = true;
	...
	if(textId == 0x4077 || // Pierre?
	   ...
	   textId == 0x40AC)
	{ // Ocarina Frog Minigame
		Interface_ChangeAlpha(1);
	}
	msgCtx->textId = textId;

	if(textId == 0x2030)
	{ // Talking to Ingo as adult in Lon Lon Ranch for the first time before freeing Epona
...
u8 Message_GetState(MessageContext* msgCtx)
{
	...
	else if(msgCtx->msgMode == MSGMODE_TEXT_DONE)
	{
		...
		else if(msgCtx->textboxEndType == TEXTBOX_ENDTYPE_2_CHOICE || msgCtx->textboxEndType == TEXTBOX_ENDTYPE_3_CHOICE)
		{
			state = TEXT_STATE_CHOICE;
		}
		else if(msgCtx->textboxEndType == TEXTBOX_ENDTYPE_EVENT || msgCtx->textboxEndType == TEXTBOX_ENDTYPE_PERSISTENT)
		{
...
void Message_DrawTextBox(GlobalContext* globalCtx, Gfx** p)
{
	...
	{
		gDPLoadTextureBlock_4b(gfx++, msgCtx->textboxSegment, G_IM_FMT_I, 128, 64, 0, G_TX_MIRROR, G_TX_NOMIRROR, 7, 0, G_TX_NOLOD, G_TX_NOLOD);
	}
	else
	{
		...
		gDPLoadTextureBlock_4b(gfx++, msgCtx->textboxSegment, G_IM_FMT_IA, 128, 64, 0, G_TX_MIRROR, G_TX_MIRROR, 7, 0, G_TX_NOLOD, G_TX_NOLOD);
	}

	gSPTextureRectangle(gfx++, R_TEXTBOX_X << 2, R_TEXTBOX_Y << 2, (R_TEXTBOX_X + R_TEXTBOX_WIDTH) << 2, (R_TEXTBOX_Y + R_TEXTBOX_HEIGHT) << 2, G_TX_RENDERTILE, 0, 0, R_TEXTBOX_TEXWIDTH << 1, R_TEXTBOX_TEXHEIGHT << 1);
	...
	if(msgCtx->textBoxType == TEXTBOX_TYPE_OCARINA)
	{
		...
		gDPLoadTextureBlock_4b(gfx++, gOcarinaTrebleClefTex, G_IM_FMT_I, 16, 32, 0, G_TX_MIRROR, G_TX_MIRROR, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
		gSPTextureRectangle(gfx++, R_TEXTBOX_CLEF_XPOS << 2, R_TEXTBOX_CLEF_YPOS << 2, (R_TEXTBOX_CLEF_XPOS + 16) << 2, (R_TEXTBOX_CLEF_YPOS + 32) << 2, G_TX_RENDERTILE, 0, 0, 1 << 10, 1 << 10);
	}
...
void Message_DrawMain(GlobalContext* globalCtx, Gfx** p)
{
	...
	if(msgCtx->msgLength != 0)
	{
		if(msgCtx->ocarinaAction != OCARINA_ACTION_FROGS && msgCtx->msgMode != MSGMODE_SONG_PLAYED_ACT && msgCtx->msgMode >= MSGMODE_TEXT_BOX_GROWING && msgCtx->msgMode < MSGMODE_TEXT_CLOSING && msgCtx->textBoxType < TEXTBOX_TYPE_NONE_BOTTOM)
		{
		...
		switch(msgCtx->msgMode)
		{
			...
			case MSGMODE_SONG_PLAYBACK_STARTING:
				...
				{
					if(msgCtx->ocarinaAction == OCARINA_ACTION_UNK_0 || msgCtx->ocarinaAction == OCARINA_ACTION_FREE_PLAY || msgCtx->ocarinaAction == OCARINA_ACTION_SCARECROW_RECORDING ||
					   msgCtx->ocarinaAction == OCARINA_ACTION_CHECK_NOWARP || msgCtx->ocarinaAction >= OCARINA_ACTION_CHECK_SARIA)
					{
						if(msgCtx->ocarinaAction == OCARINA_ACTION_FREE_PLAY || msgCtx->ocarinaAction == OCARINA_ACTION_CHECK_NOWARP)
						{
			...
			case MSGMODE_OCARINA_PLAYING:
				...
				{
					if(msgCtx->ocarinaStaff->state == OCARINA_SONG_SCARECROW || CHECK_QUEST_ITEM(QUEST_SONG_MINUET + gOcarinaSongItemMap[msgCtx->ocarinaStaff->state]))
					{
						...
						{
							if(msgCtx->ocarinaStaff->state < OCARINA_SONG_SARIAS || msgCtx->ocarinaStaff->state == OCARINA_SONG_SCARECROW)
							{
			...
			case MSGMODE_SCARECROW_RECORDING_DONE:
				r = ABS(sOcarinaNoteAPrimR - sOcarinaNoteAPrimColors[sOcarinaNoteFlashColorIdx][0]) / sOcarinaNoteFlashTimer;
				g = ABS(sOcarinaNoteAPrimG - sOcarinaNoteAPrimColors[sOcarinaNoteFlashColorIdx][1]) / sOcarinaNoteFlashTimer;
				b = ABS(sOcarinaNoteAPrimB - sOcarinaNoteAPrimColors[sOcarinaNoteFlashColorIdx][2]) / sOcarinaNoteFlashTimer;
				...
				r = ABS(sOcarinaNoteAEnvR - sOcarinaNoteAEnvColors[sOcarinaNoteFlashColorIdx][0]) / sOcarinaNoteFlashTimer;
				g = ABS(sOcarinaNoteAEnvG - sOcarinaNoteAEnvColors[sOcarinaNoteFlashColorIdx][1]) / sOcarinaNoteFlashTimer;
				b = ABS(sOcarinaNoteAEnvB - sOcarinaNoteAEnvColors[sOcarinaNoteFlashColorIdx][2]) / sOcarinaNoteFlashTimer;
				...
				r = ABS(sOcarinaNoteCPrimR - sOcarinaNoteCPrimColors[sOcarinaNoteFlashColorIdx][0]) / sOcarinaNoteFlashTimer;
				g = ABS(sOcarinaNoteCPrimG - sOcarinaNoteCPrimColors[sOcarinaNoteFlashColorIdx][1]) / sOcarinaNoteFlashTimer;
				b = ABS(sOcarinaNoteCPrimB - sOcarinaNoteCPrimColors[sOcarinaNoteFlashColorIdx][2]) / sOcarinaNoteFlashTimer;
				...
				r = ABS(sOcarinaNoteCEnvR - sOcarinaNoteCEnvColors[sOcarinaNoteFlashColorIdx][0]) / sOcarinaNoteFlashTimer;
				g = ABS(sOcarinaNoteCEnvG - sOcarinaNoteCEnvColors[sOcarinaNoteFlashColorIdx][1]) / sOcarinaNoteFlashTimer;
				b = ABS(sOcarinaNoteCEnvB - sOcarinaNoteCEnvColors[sOcarinaNoteFlashColorIdx][2]) / sOcarinaNoteFlashTimer;
			...
			case MSGMODE_SONG_PLAYED_ACT:
				...
				if(msgCtx->stateTimer == 0)
				{
					if(msgCtx->lastPlayedSong < OCARINA_SONG_SARIAS && (msgCtx->ocarinaAction < OCARINA_ACTION_PLAYBACK_MINUET || msgCtx->ocarinaAction >= OCARINA_ACTION_PLAYBACK_SARIA))
					{
					...
					else
					{
						...
						osSyncPrintf(VT_FGCOL(YELLOW));
						osSyncPrintf("☆☆☆ocarina=%d   message->ocarina_no=%d  ", msgCtx->lastPlayedSong, msgCtx->ocarinaAction);
						...
						{
							osSyncPrintf(VT_FGCOL(YELLOW));
							osSyncPrintf("Ocarina_PC_Wind=%d(%d) ☆☆☆   ", OCARINA_ACTION_CHECK_MINUET, msgCtx->ocarinaAction - OCARINA_ACTION_CHECK_MINUET);
						...
						{
							osSyncPrintf(VT_FGCOL(GREEN));
							osSyncPrintf("Ocarina_C_Wind=%d(%d) ☆☆☆   ", OCARINA_ACTION_PLAYBACK_MINUET, msgCtx->ocarinaAction - OCARINA_ACTION_PLAYBACK_MINUET);
			...
			case MSGMODE_SONG_DEMONSTRATION:
				...
				if(msgCtx->ocarinaStaff->state == 0)
				{
					...
					osSyncPrintf("onpu_buff[%d]=%x\n", msgCtx->ocarinaStaff->pos, sOcarinaNoteBuf[msgCtx->ocarinaStaff->pos]);
				}
				else
				{
			...
			case MSGMODE_SONG_PLAYBACK:
				...
				if(msgCtx->ocarinaStaff->state < OCARINA_SONG_MEMORY_GAME)
				{
					osSyncPrintf("M_OCARINA20 : ocarina_no=%x    status=%x\n", msgCtx->ocarinaAction, msgCtx->ocarinaStaff->state);
					msgCtx->lastPlayedSong = msgCtx->ocarinaStaff->state;
			...
			case MSGMODE_MEMORY_GAME_RIGHT_SKULLKID_PLAYING:
				Audio_PlaySoundGeneral(NA_SE_SY_METRONOME_LV - SFX_FLAG, &gAudioDefaultPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
				msgCtx->ocarinaStaff = Audio_OcaGetDisplayingStaff();
				if(msgCtx->ocarinaStaff->pos && sOcarinaNoteBufPos == msgCtx->ocarinaStaff->pos - 1)
				{
					sOcarinaNoteBuf[msgCtx->ocarinaStaff->pos - 1] = msgCtx->ocarinaStaff->noteIdx;
				...
				if(msgCtx->stateTimer == 0)
				{
					if(msgCtx->ocarinaStaff->state == 0)
					{
						...
						{
							Audio_PlaySoundGeneral(NA_SE_SY_METRONOME, &gAudioDefaultPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
						}
						else
						{
							Audio_PlaySoundGeneral(NA_SE_SY_METRONOME_2, &gAudioDefaultPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
						}
						msgCtx->msgMode++;
					}
				}
		...
		if(msgCtx->msgMode >= MSGMODE_OCARINA_PLAYING && msgCtx->msgMode < MSGMODE_TEXT_AWAIT_NEXT && msgCtx->ocarinaAction != OCARINA_ACTION_FREE_PLAY && msgCtx->ocarinaAction != OCARINA_ACTION_CHECK_NOWARP)
		{
			func_8009457C(&gfx);
			...
			if(msgCtx->msgMode != MSGMODE_SCARECROW_LONG_RECORDING_START && msgCtx->msgMode != MSGMODE_MEMORY_GAME_START)
			{
				for(notePosX = R_OCARINA_NOTES_XPOS, i = 0; i < 8; i++, notePosX += R_OCARINA_NOTES_XPOS_OFFSET)
				{
					...
					gDPLoadTextureBlock(gfx++, sOcarinaNoteTextures[sOcarinaNoteBuf[i]], G_IM_FMT_IA, G_IM_SIZ_8b, 16, 16, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);

					gSPTextureRectangle(gfx++, notePosX << 2, R_OCARINA_NOTES_YPOS(sOcarinaNoteBuf[i]) << 2, (notePosX + 16) << 2, (R_OCARINA_NOTES_YPOS(sOcarinaNoteBuf[i]) + 16) << 2, G_TX_RENDERTILE, 0, 0, 1 << 10, 1 << 10);
				}
			}
		}
	}
	...
	switch(msgCtx->msgMode)
	{
		case MSGMODE_TEXT_START:
			...
			if(var)
			{
				...
				{
					Actor_GetScreenPos(globalCtx, &GET_PLAYER(globalCtx)->actor, &focusScreenPosX, &playerFocusScreenPosY);
		...
		case MSGMODE_TEXT_DONE:
			...
			else if(msgCtx->textboxEndType != TEXTBOX_ENDTYPE_PERSISTENT && msgCtx->textboxEndType != TEXTBOX_ENDTYPE_EVENT && YREG(31) == 0)
			{
				if(msgCtx->textboxEndType == TEXTBOX_ENDTYPE_2_CHOICE && globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_01)
				{
					if(Message_ShouldAdvance(globalCtx))
					{
						...
						osSyncPrintf("InRaceSeq=%d(%d) OCARINA_MODE=%d  -->  ", gSaveContext.eventInf[0] & 0xF, 1, globalCtx->msgCtx.ocarinaMode);
						Message_CloseTextbox(globalCtx);
						osSyncPrintf("OCARINA_MODE=%d\n", globalCtx->msgCtx.ocarinaMode);
					}
				}
		...
		case MSGMODE_TEXT_CLOSING:
			...
			if((msgCtx->textId >= 0xC2 && msgCtx->textId < 0xC7) || (msgCtx->textId >= 0xFA && msgCtx->textId < 0xFE))
			{
				gSaveContext.healthAccumulator = 0x140; // Refill 20 hearts
			}
			if(msgCtx->textId == 0x301F || msgCtx->textId == 0xA || msgCtx->textId == 0xC || msgCtx->textId == 0xCF || msgCtx->textId == 0x21C || msgCtx->textId == 9 || msgCtx->textId == 0x4078 || msgCtx->textId == 0x2015 ||
			   msgCtx->textId == 0x3040)
			{
			...
			if(globalCtx->csCtx.state == 0)
			{
				...
				if(msgCtx->textId != 0x2061 && msgCtx->textId != 0x2025 && msgCtx->textId != 0x208C && ((msgCtx->textId < 0x88D || msgCtx->textId >= 0x893) || msgCtx->choiceIndex != 0) &&
			...
			if(msgCtx->ocarinaAction != OCARINA_ACTION_CHECK_NOWARP_DONE)
			{
				...
				if(msgCtx->ocarinaAction == OCARINA_ACTION_FREE_PLAY_DONE && (globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_01 || globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_0B))
				{
					globalCtx->msgCtx.ocarinaMode = OCARINA_MODE_04;
