...
f32 Camera_GetFloorYLayer(Camera* camera, Vec3f* norm, Vec3f* pos, s32* bgId)
{
	...
	for(i = 3; i > 0; i--)
	{
		...
		if(floorY == BGCHECK_Y_MIN || (camera->playerGroundY < floorY && !(COLPOLY_GET_NORMAL(floorPoly->normal.y) > 0.5f)))
		{
...
s32 Camera_GetWaterBoxDataIdx(Camera* camera, f32* waterY)
{
	...
	*waterY = playerPosShape.pos.y;
	...
	if(!WaterBox_GetSurface1(camera->globalCtx, &camera->globalCtx->colCtx, playerPosShape.pos.x, playerPosShape.pos.z, waterY, &waterBox))
	{
...
s16 func_80044ADC(Camera* camera, s16 yaw, s16 arg2)
{
	...
	if(arg2 || (camera->globalCtx->frames % 2) == 0)
	{
...
s32 Camera_CalcAtForLockOn(Camera* camera, VecSph* eyeAtDir, Vec3f* targetPos, f32 yOffset, f32 distance, f32* yPosOffset, VecSph* outPlayerToTargetDir, s16 flags)
{
	...
	else
	{
		...
		playerToTargetDir.r = (playerToTargetDir.r * PCT(OREG(39))) - (((PCT(OREG(39)) - PCT(OREG(38))) * playerToTargetDir.r) * (playerToTargetDir.r / distance));
	...
	if(PREG(89))
	{
		osSyncPrintf("%f (%f %f %f) %f\n", playerToTargetDir.r / distance, lookFromOffset.x, lookFromOffset.y, lookFromOffset.z, camera->atLERPStepScale);
	}
...
s32 Camera_CalcAtForHorse(Camera* camera, VecSph* eyeAtDir, f32 yOffset, f32* yPosOffset, s16 calcSlope)
{
	...
	if(calcSlope != 0)
	{
		posOffsetTarget.y -= Camera_CalcSlopeYAdj(&camera->floorNorm, camera->playerPosRot.rot.y, eyeAtDir->yaw, OREG(9));
	}
...
s32 Camera_Normal1(Camera* camera)
{
	...
	{
		camera->yawUpdateRateInv = Camera_LERPCeilF(anim->swing.swingUpdateRate + (f32)(anim->swing.swingUpdateRateTimer * 2), camera->yawUpdateRateInv, sp98, rate);
		camera->pitchUpdateRateInv = Camera_LERPCeilF((f32)R_CAM_DEFA_PHI_UPDRATE + (f32)(anim->swing.swingUpdateRateTimer * 2), camera->pitchUpdateRateInv, sp9C, rate);
		anim->swing.swingUpdateRateTimer--;
	}
	else
	{
		camera->yawUpdateRateInv = Camera_LERPCeilF(anim->swing.swingUpdateRate - ((OREG(49) * 0.01f) * anim->swing.swingUpdateRate * sp94), camera->yawUpdateRateInv, sp98, rate);
	...
	if((camera->status == CAM_STAT_ACTIVE) && (!(norm1->interfaceFlags & 0x10)))
	{
		...
		{
			camera->inputDir.y = Camera_LERPCeilS(camera->inputDir.y + BINANG_SUB(BINANG_ROT180(anim->swing.unk_16), camera->inputDir.y), camera->inputDir.y, 1.0f - (0.99f * sp98), 0xA);
		}
		...
		// crit wiggle
		if(gSaveContext.health <= 16 && ((camera->globalCtx->frames % 256) == 0))
		{
...
s32 Camera_Normal2(Camera* camera)
{
	...
	if((phi_a1 < 0x4000 && ABS(phi_a0) > phi_a1) || (phi_a1 >= 0x4000 && ABS(phi_a0) < phi_a1))
	{
		...
		phi_a0 += sp88.yaw;
		adjSph.yaw = Camera_LERPCeilS(phi_a0, atToEyeDir.yaw, (1.0f / camera->yawUpdateRateInv) * camera->speedRatio, 0xA);
		if(anim->unk_28 & 1)
		{
...
s32 Camera_Normal3(Camera* camera)
{
	...
	if(anim->swing.swingUpdateRateTimer != 0)
	{
		camera->yawUpdateRateInv = Camera_LERPCeilF(norm3->yawUpdateSpeed + (anim->swing.swingUpdateRateTimer * 2), camera->yawUpdateRateInv, sp98, 0.1f);
		camera->pitchUpdateRateInv = Camera_LERPCeilF((f32)R_CAM_DEFA_PHI_UPDRATE + (anim->swing.swingUpdateRateTimer * 2), camera->pitchUpdateRateInv, sp94, 0.1f);
		anim->swing.swingUpdateRateTimer--;
	}
	else
	{
...
s32 Camera_Jump1(Camera* camera)
{
	...
	if((camera->status == CAM_STAT_ACTIVE) && !(jump1->interfaceFlags & 0x10))
	{
		...
		if(anim->swing.unk_18)
		{
			camera->inputDir.y = Camera_LERPCeilS(camera->inputDir.y + BINANG_SUB(BINANG_ROT180(anim->swing.unk_16), camera->inputDir.y), camera->inputDir.y, 1.0f - (0.99f * spA4), 0xA);
		}
	}
	else
	{
...
s32 Camera_Jump3(Camera* camera)
{
	...
	if((camera->status == CAM_STAT_ACTIVE) && !(jump3->interfaceFlags & 0x10))
	{
		...
		if(anim->swing.unk_18 != 0)
		{
			camera->inputDir.y = Camera_LERPCeilS(camera->inputDir.y + BINANG_SUB(BINANG_ROT180(anim->swing.unk_16), camera->inputDir.y), camera->inputDir.y, 1.0f - (0.99f * spBC), 0xA);
		}
	}
	else
	{
...
s32 Camera_Battle1(Camera* camera)
{
	...
	if(camera->player->stateFlags1 & PLAYER_STATE_SWORD_CHARGE)
	{
		// charging sword.
	...
	if(camera->target == NULL || camera->target->update == NULL)
	{
		...
		{
			osSyncPrintf(VT_COL(YELLOW, BLACK) "camera: warning: battle: target is not valid, change parallel\n" VT_RST);
		}
		camera->target = NULL;
	...
	Camera_CalcAtForLockOn(camera, &atToEyeNextDir, &camera->targetPosRot.pos, isOffGround ? batt1->yOffsetOffGround : batt1->yOffset, distance, &anim->yPosOffset, &playerToTargetDir, (isOffGround ? 0x81 : 1) | batt1->flags);
	...
	if(anim->animTimer != 0)
	{
		if(anim->animTimer >= OREG(24))
		{
			...
			spB4.pitch = Camera_LERPCeilS(playerToTargetDir.pitch + (tmpAng2 * sp86), atToEyeDir.pitch, PCT(OREG(28)), 0xA);
		}
		else
		{
			skipEyeAtCalc = true;
	...
	camera->fov = Camera_LERPCeilF((player->swordState != 0 ? 0.8f : gSaveContext.health <= 0x10 ? 0.8f : 1.0f) * (fov - ((fov * 0.05f) * distRatio)), camera->fov, camera->fovUpdateRate, 1.0f);
...
s32 Camera_KeepOn1(Camera* camera)
{
	...
	switch(camera->paramFlags & 0x18)
	{
		...
		cont:
			...
			Camera_CalcAtForLockOn(camera, &spB8, &camera->targetPosRot.pos, sp80 ? keep1->unk_28 : keep1->unk_00, sp104, &anim->unk_08, &spC8, (sp80 ? 0x80 : 0) | keep1->interfaceFlags);
...
s32 Camera_KeepOn3(Camera* camera)
{
	...
	if(camera->animState == 0 || camera->animState == 0xA || camera->animState == 0x14)
	{
		...
		if(!(keep3->flags & 0x80))
		{
			while(i < angleCnt)
			{
				if(!CollisionCheck_LineOCCheck(camera->globalCtx, &camera->globalCtx->colChkCtx, &anim->atTarget, &lineChkPointB, colChkActors, 2) && !Camera_BGCheck(camera, &anim->atTarget, &lineChkPointB))
				{
					break;
				}
	...
	if(camera->unk_14C & 8)
	{
		...
		if(camera->xzSpeed > 0.001f || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_A) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_B) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CLEFT) ||
		   CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CDOWN) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CUP) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CRIGHT) ||
		   CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_R) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_Z))
		{
			camera->unk_14C |= 4;
			camera->unk_14C &= ~8;
		}
	...
	if(RELOAD_PARAMS)
	{
		...
		switch(*temp_s0)
		{
			...
			case 12:
				...
				keep4->unk_08 = -2.0f;
				keep4->unk_0C = 120.0f;
				keep4->unk_10 = player->stateFlags1 & PLAYER_STATE_SWIMMING ? 0.0f : 20.0f; // Swimming?
				keep4->unk_1C = 0x3212;
				keep4->unk_1E = 0x1E;
				keep4->unk_18 = 50.0f;
				break;
	...
	switch(camera->animState)
	{
		...
		case 0x14:
			...
			if(keep4->unk_1C & 2)
			{
				...
				spA0 = BINANG_SUB(BINANG_ROT180(playerPosRot->rot.y), spA8.yaw) > 0 ? BINANG_ROT180(playerPosRot->rot.y) + DEGF_TO_BINANG(keep4->unk_0C) : BINANG_ROT180(playerPosRot->rot.y) - DEGF_TO_BINANG(keep4->unk_0C);
			}
			...
			else if((keep4->unk_1C & 8) && camera->target != NULL)
			{
				...
				spA0 = BINANG_SUB(BINANG_ROT180(sp60.rot.y), spA8.yaw) > 0 ? BINANG_ROT180(sp60.rot.y) + DEGF_TO_BINANG(keep4->unk_0C) : BINANG_ROT180(sp60.rot.y) - DEGF_TO_BINANG(keep4->unk_0C);
			...
			else if((keep4->unk_1C & 0x80) && camera->target != NULL)
			{
				...
				spA0 = (BINANG_SUB(sp9E, spA8.yaw) > 0) ? sp9E + DEGF_TO_BINANG(keep4->unk_0C) : sp9E - DEGF_TO_BINANG(keep4->unk_0C);
				spCC[1] = camera->target;
				sp9C++;
			}
			...
			if(!(keep4->unk_1C & 1))
			{
				...
				{
					if(!CollisionCheck_LineOCCheck(camera->globalCtx, &camera->globalCtx->colChkCtx, &D_8015BD50, &D_8015BD70, spCC, sp9C) && !Camera_BGCheck(camera, &D_8015BD50, &D_8015BD70))
					{
						break;
					}
...
s32 Camera_Fixed4(Camera* camera)
{
	...
	*eye = *eyeNext;
	...
	atEyeNextOffset.pitch = Camera_LERPCeilS(atTargetEyeNextOffset.pitch, atEyeNextOffset.pitch, anim->followSpeed * camera->speedRatio, 0xA);
	atEyeNextOffset.yaw = Camera_LERPCeilS(atTargetEyeNextOffset.yaw, atEyeNextOffset.yaw, anim->followSpeed * camera->speedRatio, 0xA);
...
s32 Camera_Data4(Camera* camera)
{
	...
	atOffset.yaw = (initParams->jfifId & 1) ? (DEGF_TO_BINANG(camera->data2) + initParams->eyePosRot.rot.y) : eyeAtOffset.yaw;
	atOffset.pitch = (initParams->jfifId & 2) ? (DEGF_TO_BINANG(camera->data3) + initParams->eyePosRot.rot.x) : eyeAtOffset.pitch;
...
s32 Camera_Unique0(Camera* camera)
{
	...
	if(params->interfaceFlags & 1)
	{
		if(anim->animTimer > 0)
		{
		    ...
		    ((OLib_Vec3fDistXZ(&playerPosRot->pos, &anim->initalPos) >= 10.0f) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_A) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_B) ||
		     CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CLEFT) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CDOWN) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CUP) ||
		     CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CRIGHT) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_R) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_Z)))
		{
	...
	else
	{
		...
		if((!(player->stateFlags1 & PLAYER_STATE1_29)) &&
		   ((0.001f < camera->xzSpeed) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_A) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_B) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CLEFT) ||
		    CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CDOWN) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CUP) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CRIGHT) ||
		    CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_R) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_Z)))
		{
...
s32 Camera_Unique7(Camera* camera)
{
	...
	*eye = *eyeNext;
	...
	playerPosEyeOffset.pitch = -BGCAM_ROT(sceneCamData).x * Math_CosS(playerPosEyeOffset.yaw - BGCAM_ROT(sceneCamData).y);
...
s32 Camera_OnePoint_Unique9(Camera* camera)
{
	...
	if(anim->keyFrameTimer == 0)
	{
		...
		if(anim->curKeyFrameIdx < ONEPOINT_CS_INFO(camera)->keyFrameCnt)
		{
			...
			if(anim->curKeyFrame->unk_01 != 0xFF)
			{
				...
				else if(camera->player->stateFlags1 & PLAYER_STATE_SWIMMING && player->currentBoots != PLAYER_BOOTS_IRON)
				{
	...
	else if(eyeInitFlags == 0x400 || eyeInitFlags == (s16)0x8400 || eyeInitFlags == 0x500 || eyeInitFlags == (s16)0x8500)
	{
		if(camera->target != NULL && camera->target->update != NULL)
		{
			Actor_GetFocus(&targethead, camera->target);
	...
	*eye = *eyeNext;
	...
	if(anim->curKeyFrame->actionFlags & 0x40)
	{
		...
		// Swimming without iron boots?
		if(camera->player->stateFlags1 & PLAYER_STATE_SWIMMING && player->currentBoots != PLAYER_BOOTS_IRON)
		{
...
static void Camera_DebugPrintSplineArray(const char* name, s16 length, CutsceneCameraPoint cameraPoints[])
{
	...
	for(i = 0; i < length; i++)
	{
		...
		osSyncPrintf("    /*     pos      */ { %d, %d, %d }\n", cameraPoints[i].pos.x, cameraPoints[i].pos.y, cameraPoints[i].pos.z);
		osSyncPrintf("    },\n");
	}
	osSyncPrintf("};\n\n");
}
...
s32 Camera_Demo1(Camera* camera)
{
	...
	switch(camera->animState)
	{
		case 0:
			...
			// camera->globalCtx->frames, *relativeToPlayer == 0 ? "absolute" : "relative");
		...
		case 1:
			...
			if(func_800BB2B4(&csEyeUpdate, &newRoll, cameraFOV, csEyePoints, &anim->keyframe, &anim->curFrame) || func_800BB2B4(&csAtUpdate, &newRoll, cameraFOV, csAtPoints, &anim->keyframe, &anim->curFrame))
			{
				camera->animState++;
			}
			if(*relativeToPlayer)
...
s32 Camera_Demo5(Camera* camera)
{
	...
	pad = sDemo5PrevSfxFrame - camera->globalCtx->frames.whole();
	if((pad >= 0x33) || (pad < -0x32))
	{
		Common_PlaySfx((u16)(uint64_t)camera->data1);
	}

	sDemo5PrevSfxFrame = camera->globalCtx->frames.whole();
	...
	else
	{
		...
		if(camera->target->category == ACTORCAT_PLAYER)
		{
			pad = camera->globalCtx->frames.whole() - sDemo5PrevAction12Frame;
	...
	switch(camera->animState)
	{
		...
		case 1:
			...
			if(anim->animTimer > 0)
			{
				...
				if(func_800BB2B4(&csEyeUpdate, &newRoll, camFOV, demo9OnePoint->onePointCs.eyePoints, &anim->keyframe, &anim->curFrame) != 0 ||
				   func_800BB2B4(&csAtUpdate, &newRoll, camFOV, demo9OnePoint->onePointCs.atPoints, &anim->keyframe, &anim->curFrame) != 0)
				{
					camera->animState = 2;
				}
			...
			if(anim->finishAction != 0)
			{
				if(anim->finishAction != 0x1000)
				{
					if(anim->finishAction == 0x2000)
					{
						...
						onePointTimer = demo9OnePoint->onePointCs.initTimer < 50 ? 5 : demo9OnePoint->onePointCs.initTimer / 5;
...
s32 Camera_Special5(Camera* camera)
{
	...
	else if(anim->animTimer == 0)
	{
		...
		if(spec5->minDistForRot < OLib_Vec3fDist(&spA8.pos, &playerPosRot->pos))
		{
			...
			sp74.yaw = BINANG_ROT180(playerPosRot->rot.y) + (s16)(spA4 < 0 ? -(s16)(0x1553 + (s16)(temp_f0_2 * 2730.0f)) : (s16)(0x1553 + (s16)(temp_f0_2 * 2730.0f)));
...
s32 Camera_Special9(Camera* camera)
{
	...
	switch(camera->animState)
	{
		case 0:
			camera->unk_14C &= ~(0x4 | 0x2);
			camera->animState++;
			anim->targetYaw = ABS(playerPosRot->rot.y - adjustedPlayerPosRot.rot.y) >= 0x4000 ? BINANG_ROT180(adjustedPlayerPosRot.rot.y.whole()) : adjustedPlayerPosRot.rot.y.whole();
		case 1:
			...
			if(spec9->doorParams.timer1 <= 0)
			{
				...
				else
				{
					...
					// 0xAAA ~ 15 degrees.
					yaw = 0xAAA * ((camera->globalCtx->frames & 1) ? 1 : -1);
		...
		default:
			...
			if(camera->xzSpeed > 0.001f || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_A) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_B) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CLEFT) ||
			   CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CDOWN) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CUP) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_CRIGHT) ||
			   CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_R) || CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_Z) || params->interfaceFlags & 0x8)
			{
				Camera_ChangeSettingFlags(camera, camera->prevSetting, 2);
				camera->unk_14C |= (0x4 | 0x2);
			}
...
void Camera_Init(Camera* camera, View* view, CollisionContext* colCtx, GlobalContext* globalCtx)
{
	...
	camera->globalCtx = globalCtxAlt = globalCtx;
...
void Camera_PrintSettings(Camera* camera)
{
	...
	if((OREG(0) & 1) && (camera->globalCtx->activeCamera == camera->thisIdx) && !gDbgCamEnabled)
	{
		...
		func_8006376C(5, 0x19, 4, sCameraFunctionNames[sCameraSettings[camera->setting].cameraModes[camera->mode].funcIdx]);
...
s32 Camera_DbgChangeMode(Camera* camera)
{
	...
	{
		if(CHECK_BTN_ALL(globalCtxAlt->input[2].press.button, BTN_CUP))
		{
			osSyncPrintf("attention sound URGENCY\n");
			Common_PlaySfx(NA_SE_SY_ATTENTION_URGENCY);
		}
		if(CHECK_BTN_ALL(globalCtxAlt->input[2].press.button, BTN_CDOWN))
		{
			osSyncPrintf("attention sound NORMAL\n");
			Common_PlaySfx(NA_SE_SY_ATTENTION_ON);
		}

		if(CHECK_BTN_ALL(globalCtxAlt->input[2].press.button, BTN_CRIGHT))
		{
			changeDir = 1;
		}
		if(CHECK_BTN_ALL(globalCtxAlt->input[2].press.button, BTN_CLEFT))
		{
			changeDir = -1;
		}
		if(changeDir != 0)
...
Vec3s Camera_Update(Camera* camera)
{
	...
	if(camera->player != NULL)
	{
		...
		playerGroundY = BgCheck_EntityRaycastFloor5(camera->globalCtx, &camera->globalCtx->colCtx, &playerFloorPoly, &bgId, &camera->player->actor, &spAC);
		...
		if(sOOBTimer < 200)
		{
			...
			if((camera->unk_14C & 1) && (camera->unk_14C & 4) && (!(camera->unk_14C & 0x400)) && (!(camera->unk_14C & 0x200) || (player->currentBoots == PLAYER_BOOTS_IRON)) && (!(camera->unk_14C & (s16)0x8000)) &&
			   (playerGroundY != BGCHECK_Y_MIN))
			{
			...
			if(camera->nextCamDataIdx != -1 && (fabsf(curPlayerPosRot.pos.y - playerGroundY) < 2.0f) && (!(camera->unk_14C & 0x200) || (player->currentBoots == PLAYER_BOOTS_IRON)))
			{
	...
	{
		osSyncPrintf(
		    "camera: engine (%s(%d) %s(%d) %s(%d)) ok!\n", &sCameraSettingNames[camera->setting], camera->setting, &sCameraModeNames[camera->mode], camera->mode,
		    &sCameraFunctionNames[sCameraSettings[camera->setting].cameraModes[camera->mode].funcIdx], sCameraSettings[camera->setting].cameraModes[camera->mode].funcIdx);
	}

	// enable/disable debug cam
	if(CHECK_BTN_ALL(globalCtxAlt->input[2].press.button, BTN_START))
	{
		gDbgCamEnabled ^= 1;
		if(gDbgCamEnabled)
		{
	...
	if(PREG(81))
	{
		osSyncPrintf("dir  (%d) %d(%f) %d(%f) 0(0) \n", sUpdateCameraDirection, camera->inputDir.x, BINANG_TO_DEGF(camera->inputDir.x), camera->inputDir.y, BINANG_TO_DEGF(camera->inputDir.y));
		osSyncPrintf("real (%d) %d(%f) %d(%f) 0(0) \n", sUpdateCameraDirection, camera->camDir.x, BINANG_TO_DEGF(camera->camDir.x), camera->camDir.y, BINANG_TO_DEGF(camera->camDir.y));
	}

	if(camera->timer != -1 && CHECK_BTN_ALL(globalCtxAlt->input[0].press.button, BTN_DRIGHT))
	{
		camera->timer = 0;
	}
...
s16 Camera_ChangeSettingFlags(Camera* camera, s16 setting, s16 flags)
{
	...
	if(((setting == CAM_SET_MEADOW_BIRDS_EYE) || (setting == CAM_SET_MEADOW_UNUSED)) && LINK_IS_ADULT && (camera->globalCtx->sceneNum == SCENE_SPOT05))
	{
		camera->unk_14A |= 0x10;
		return -5;
	}
