...
#define DEFINE_ACTOR(name, _1, allocType) {(uintptr_t)_ovl_##name##SegmentRomStart, (uintptr_t)_ovl_##name##SegmentRomEnd, _ovl_##name##SegmentStart, _ovl_##name##SegmentEnd, NULL, &name##_InitVars, #name, allocType, 0},
...
void ActorOverlayTable_LogPrint(void)
{
	...
	{
		osSyncPrintf(
		    "%08x %08x %08x %08x %08x %08x %s\n", overlayEntry->vromStart, overlayEntry->vromEnd, overlayEntry->vramStart, overlayEntry->vramEnd, overlayEntry->loadedRamAddr, &overlayEntry->initInfo->id,
