...
void* AudioLoad_DmaSampleData(Pointer devAddr, size_t size, s32 arg2, u8* dmaIndexRef, s32 medium)
{
	...
	if(arg2 != 0 || *dmaIndexRef >= gAudioContext.sampleDmaListSize1)
	{
		for(i = gAudioContext.sampleDmaListSize1; i < gAudioContext.sampleDmaCount; i++)
		{
			...
			if(0 <= bufferPos && (u32)bufferPos <= dma->size - size)
			{
				...
				if(dma->ttl == 0 && gAudioContext.sampleDmaReuseQueue2RdPos != gAudioContext.sampleDmaReuseQueue2WrPos)
				{
					// Move the DMA out of the reuse queue, by swapping it with the
					// read pos, and then incrementing the read pos.
					if(dma->reuseIndex != gAudioContext.sampleDmaReuseQueue2RdPos)
					{
						gAudioContext.sampleDmaReuseQueue2[dma->reuseIndex] = gAudioContext.sampleDmaReuseQueue2[gAudioContext.sampleDmaReuseQueue2RdPos];
						gAudioContext.sampleDmas[gAudioContext.sampleDmaReuseQueue2[gAudioContext.sampleDmaReuseQueue2RdPos]].reuseIndex = dma->reuseIndex;
					}
	...
	again:
		...
		if(0 <= bufferPos && (u32)bufferPos <= dma->size - size)
		{
			...
			if(dma->ttl == 0)
			{
				...
				{
					gAudioContext.sampleDmaReuseQueue1[dma->reuseIndex] = gAudioContext.sampleDmaReuseQueue1[gAudioContext.sampleDmaReuseQueue1RdPos];
					gAudioContext.sampleDmas[gAudioContext.sampleDmaReuseQueue1[gAudioContext.sampleDmaReuseQueue1RdPos]].reuseIndex = dma->reuseIndex;
				}
	...
	AudioLoad_Dma(nullptr, OS_MESG_PRI_NORMAL, OS_READ, dmaDevAddr.get(), dma->ramAddr, transfer, nullptr, medium, "SUPERDMA");
...
void AudioLoad_InitSampleDmaBuffers(s32 arg0)
{
	...
	gAudioContext.sampleDmas = (SampleDma*)AudioHeap_Alloc(&gAudioContext.notesAndBuffersPool, 4 * gAudioContext.numNotes * sizeof(SampleDma) * gAudioContext.audioBufferParameters.specUnk4);
	...
	for(i = 0; i < t2; i++)
	{
		...
		dma->ramAddr = (u8*)AudioHeap_AllocAttemptExternal(&gAudioContext.notesAndBuffersPool, gAudioContext.sampleDmaBufSize);
		if(dma->ramAddr == NULL)
		{
			break;
		}
...
void AudioLoad_SetSampleFontLoadStatusAndApplyCaches(s32 sampleBankId, s32 status)
{
	if(sampleBankId != 0xFF)
	{
		...
		if((gAudioContext.sampleFontLoadStatus[sampleBankId] == 5) || (gAudioContext.sampleFontLoadStatus[sampleBankId] == 2))
		{
...
s32 AudioLoad_SyncLoadSample(SoundFontSample* sample, s32 fontId)
{
	...
	if(sample->unk_bit25 == 1)
	{
		...
		{
			sampleAddr = AudioHeap_AllocSampleCache(sample->size, fontId, (void*)sample->sampleAddr, sample->medium, CACHE_PERSISTENT);
			if(sampleAddr == NULL)
			{
				return -1;
			}
			...
			{
				AudioLoad_SyncDmaUnkMedium(sample->sampleAddr, sampleAddr, sample->size, gAudioContext.sampleBankTable->unkMediumParam);
			}
			else
			{
...
void* AudioLoad_AsyncLoadInner(s32 tableType, s32 id, s32 nChunks, s32 retData, OSMesgQueue* retQueue)
{
	...
	else
	{
		...
		if(medium == MEDIUM_UNK)
		{
			AudioLoad_StartAsyncLoadUnkMedium((s16)sp50->unkMediumParam, devAddr, ret, size, medium, nChunks, retQueue, MK_ASYNC_MSG(retData, tableType, id, status));
		}
		else
		{
...
s32 AudioLoad_SlowLoadSample(s32 fontId, s32 instId, s8* isDone)
{
	...
	slowLoad->curRamAddr = (u8*)AudioHeap_AllocSampleCache(sample->size, fontId, sample->sampleAddr, sample->medium, CACHE_TEMPORARY);
			...
			case LOAD_STATUS_START:
				...
				else if(slowLoad->bytesRemaining < 0x400)
				{
					if(slowLoad->medium == MEDIUM_UNK)
					{
						...
						AudioLoad_DmaSlowCopyUnkMedium(slowLoad->curDevAddr, slowLoad->curRamAddr, size, slowLoad->unkMediumParam);
					}
					else
					{
				...
				else
				{
					if(slowLoad->medium == MEDIUM_UNK)
					{
						AudioLoad_DmaSlowCopyUnkMedium(slowLoad->curDevAddr, slowLoad->curRamAddr, 0x400, slowLoad->unkMediumParam);
					}
					else
					{
...
void AudioLoad_DmaSlowCopy(AudioSlowLoad* slowLoad, s32 size)
{
	...
	AudioLoad_Dma(&slowLoad->ioMesg, OS_MESG_PRI_NORMAL, 0, slowLoad->curDevAddr, slowLoad->curRamAddr, size, nullptr, slowLoad->medium, "SLOWCOPY");
}
...
void AudioLoad_ProcessAsyncLoad(AudioAsyncLoad* asyncLoad, s32 resetStatus)
{
	...
	if(asyncLoad->bytesRemaining < asyncLoad->chunkSize)
	{
		if(asyncLoad->medium == MEDIUM_UNK)
		{
			AudioLoad_AsyncDmaUnkMedium(asyncLoad->curDevAddr, asyncLoad->curRamAddr, asyncLoad->bytesRemaining, sampleBankTable->unkMediumParam);
		}
		else
		{
	...
	if(asyncLoad->medium == MEDIUM_UNK)
	{
		AudioLoad_AsyncDmaUnkMedium(asyncLoad->curDevAddr, asyncLoad->curRamAddr, asyncLoad->chunkSize, sampleBankTable->unkMediumParam);
	}
	else
	{
...
void AudioLoad_RelocateFontAndPreloadSamples(s32 fontId, SoundFontData* mem, RelocInfo* relocInfo, s32 async)
{
	...
	for(i = 0; i < gAudioContext.numUsedSamples; i++)
	{
		...
		switch(async)
		{
			case false:
				...
				{
					addr = (u8*)AudioHeap_AllocSampleCache(sample->size, relocInfo->sampleBankId1, sample->sampleAddr, sample->medium, CACHE_PERSISTENT);
				}
				else if(sample->medium == relocInfo->medium2)
				{
					addr = (u8*)AudioHeap_AllocSampleCache(sample->size, relocInfo->sampleBankId2, sample->sampleAddr, sample->medium, CACHE_PERSISTENT);
				}
				else if(sample->medium == MEDIUM_DISK_DRIVE)
				{
					addr = (u8*)AudioHeap_AllocSampleCache(sample->size, 0xFE, sample->sampleAddr, sample->medium, CACHE_PERSISTENT);
				}
				break;
			...
			case true:
				if(sample->medium == relocInfo->medium1)
				{
					addr = (u8*)AudioHeap_AllocSampleCache(sample->size, relocInfo->sampleBankId1, sample->sampleAddr, sample->medium, CACHE_TEMPORARY);
				}
				else if(sample->medium == relocInfo->medium2)
				{
					addr = (u8*)AudioHeap_AllocSampleCache(sample->size, relocInfo->sampleBankId2, sample->sampleAddr, sample->medium, CACHE_TEMPORARY);
				}
				else if(sample->medium == MEDIUM_DISK_DRIVE)
				{
					addr = (u8*)AudioHeap_AllocSampleCache(sample->size, 0xFE, sample->sampleAddr, sample->medium, CACHE_TEMPORARY);
				}
				break;
		}
		if(addr == NULL)
		...
		switch(async)
		{
			case false:
				if(sample->medium == MEDIUM_UNK)
				{
					AudioLoad_SyncDmaUnkMedium((u32)sample->sampleAddr, addr, sample->size, gAudioContext.sampleBankTable->unkMediumParam);
	...
	if(gAudioContext.preloadSampleStackTop != 0 && !preloadInProgress)
	{
		...
		AudioLoad_StartAsyncLoad((u32)sample->sampleAddr, topPreload->ramAddr, sample->size, sample->medium, nChunks, nullptr, topPreload->encodedInfo);
	}
}
...
s32 AudioLoad_ProcessSamplePreloads(s32 resetStatus)
{
	...
	if(gAudioContext.preloadSampleStackTop > 0)
	{
		...
		for(;;)
		{
			...
			{
				AudioLoad_StartAsyncLoad((u32)sample->sampleAddr, preload->ramAddr, sample->size, sample->medium, nChunks, nullptr, preload->encodedInfo);
				break;
			}
		}
	}
