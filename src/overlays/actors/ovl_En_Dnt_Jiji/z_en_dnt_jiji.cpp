...
void EnDntJiji_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gDntJijiSkel, &gDntJijiBurrowAnim, pthis->jointTable, pthis->morphTable, 13);
...
void EnDntJiji_Wait(EnDntJiji* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->timer == 1) && (pthis->actor.xzDistToPlayer < 150.0f) && !Gameplay_InCsMode(globalCtx) && !(player->stateFlags1 & PLAYER_STATE1_11))
	{
