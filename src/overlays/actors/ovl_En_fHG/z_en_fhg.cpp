...
static EnfHGPainting sPaintings[] = {
    {{0.0f, 60.0f, -315.0f}, 0x0000}, {{-260.0f, 60.0f, -145.0f}, 0x2AAA}, {{-260.0f, 60.0f, 165.0f}, 0x5554}, {{0.0f, 60.0f, 315.0f}, 0x7FFE}, {{260.0f, 60.0f, 155.0f}, 0xAAA8}, {{260.0f, 60.0f, -155.0f}, 0xD552},
};
...
void EnfHG_Intro(EnfHG* pthis, GlobalContext* globalCtx)
{
	...
	{
		case INTRO_WAIT:
			if((fabsf(player->actor.world.pos.x - (GND_BOSSROOM_CENTER_X + 0.0f)) < 150.0f) && (fabsf(player->actor.world.pos.z - (GND_BOSSROOM_CENTER_Z + 0.0f)) < 150.0f))
			{
				pthis->cutsceneState = INTRO_READY;
			}
			break;
		case INTRO_READY:
			if((fabsf(player->actor.world.pos.x - (GND_BOSSROOM_CENTER_X + 0.0f)) < 100.0f) && (fabsf(player->actor.world.pos.z - (GND_BOSSROOM_CENTER_Z + 315.0f)) < 100.0f))
			{
		...
		case INTRO_START:
			if(gSaveContext.eventChkInf[7] & 4)
			{
				...
				{
					Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DOOR_SHUTTER, GND_BOSSROOM_CENTER_X + 0.0f, GND_BOSSROOM_CENTER_Y - 97.0f, GND_BOSSROOM_CENTER_Z + 308.0f, 0, 0, 0, (SHUTTER_PG_BARS << 6));
				}
		...
		case INTRO_CUT:
			...
			pthis->timers[0] = 250;
		case INTRO_LAUGH:
			Math_ApproachF(&pthis->cameraEye.x, GND_BOSSROOM_CENTER_X + 20.0f, 0.05f, pthis->cameraSpeedMod * pthis->cameraEyeVel.x);
			Math_ApproachF(&pthis->cameraEye.y, GND_BOSSROOM_CENTER_Y + 102.0f, 0.05f, pthis->cameraSpeedMod * pthis->cameraEyeVel.y);
			Math_ApproachF(&pthis->cameraEye.z, GND_BOSSROOM_CENTER_Z + 25.0f, 0.05f, pthis->cameraSpeedMod * pthis->cameraEyeVel.z);
			Math_ApproachF(&pthis->cameraAt.x, GND_BOSSROOM_CENTER_X - 150.0f, 0.05f, pthis->cameraSpeedMod * pthis->cameraAtVel.x);
			Math_ApproachF(&pthis->cameraAt.y, GND_BOSSROOM_CENTER_Y + 197.0f, 0.05f, pthis->cameraSpeedMod * pthis->cameraAtVel.y);
			Math_ApproachF(&pthis->cameraAt.z, GND_BOSSROOM_CENTER_Z - 65.0f, 0.05f, pthis->cameraSpeedMod * pthis->cameraAtVel.z);
			...
			if(pthis->timers[0] == 212)
			{
				...
				Animation_Change(&pthis->skin.skelAnime, &gPhantomHorseIdleAnim, 0.3f, 0.0f, 5.0f, ANIMMODE_LOOP_INTERP, -10.0f);
			}
			...
			{
				Common_PlaySfxAtPos(&audioVec_43, NA_SE_EN_FANTOM_EYE);
			}
			if(pthis->timers[0] == 130)
			{
		...
		case INTRO_TITLE:
			if(pthis->timers[1] == 1)
			{
				Animation_Change(&pthis->skin.skelAnime, &gPhantomHorseIdleAnim, 0.5f, 0.0f, Animation_GetLastFrame(&gPhantomHorseIdleAnim), ANIMMODE_LOOP_INTERP, -3.0f);
			}
			Math_ApproachF(&pthis->cameraEye.x, GND_BOSSROOM_CENTER_X + 180.0f, 0.1f, pthis->cameraSpeedMod * pthis->cameraEyeVel.x);
			Math_ApproachF(&pthis->cameraEye.y, GND_BOSSROOM_CENTER_Y + 7.0f, 0.1f, pthis->cameraSpeedMod * pthis->cameraEyeVel.y);
			Math_ApproachF(&pthis->cameraEye.z, pthis->cameraPanZ + (GND_BOSSROOM_CENTER_Z + 140.0f), 0.1f, pthis->cameraSpeedMod * pthis->cameraEyeVel.z);
			...
			Math_ApproachF(&pthis->cameraAt.y, (pthis->actor.world.pos.y + 70.0f) - 20.0f, 0.1f, pthis->cameraSpeedMod * 10.0f);
			...
			{
				TitleCard_InitBossName(globalCtx, &globalCtx->actorCtx.titleCtx, SEGMENTED_TO_VIRTUAL(gPhantomGanonTitleCardTex), 160, 180, 128, 40);
			}
			if(pthis->timers[0] == 0)
			{
				...
				pthis->timers[1] = 23;
				pthis->cameraSpeedMod = 0.0f;
				Animation_Change(&pthis->skin.skelAnime, &gPhantomHorseLeapAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gPhantomHorseLeapAnim), ANIMMODE_ONCE_INTERP, -4.0f);
		...
		case INTRO_RETREAT:
			if(pthis->timers[1] == 1)
			{
				Animation_Change(&pthis->skin.skelAnime, &gPhantomHorseLandAnim, 0.5f, 0.0f, Animation_GetLastFrame(&gPhantomHorseLandAnim), ANIMMODE_ONCE_INTERP, -3.0f);
			...
			Math_ApproachF(&pthis->cameraEye.z, pthis->cameraPanZ + (GND_BOSSROOM_CENTER_Z + 100.0f), 0.1f, pthis->cameraSpeedMod * 1.5f);
			Math_ApproachF(&pthis->cameraPanZ, -100.0f, 0.1f, 1.0f);
			Math_ApproachF(&pthis->actor.world.pos.z, GND_BOSSROOM_CENTER_Z + 400.0f - 0.5f, 1.0f, pthis->cameraSpeedMod * 10.0f);
			Math_ApproachF(&pthis->cameraSpeedMod, 1.0f, 1.0f, 0.05f);
			if((fabsf(pthis->actor.world.pos.z - (GND_BOSSROOM_CENTER_Z + 400.0f - 0.5f)) < 300.0f) && !pthis->spawnedWarp)
			{
				pthis->spawnedWarp = true;
				Actor_SpawnAsChild(
				    &globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_FHG_FIRE, GND_BOSSROOM_CENTER_X + 0.0f, pthis->actor.world.pos.y + 50.0f, GND_BOSSROOM_CENTER_Z + 400.0f - 0.5f, 0, pthis->actor.shape.rot.y, 0,
...
void EnfHG_Approach(EnfHG* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.params == GND_REAL_BOSS)
	{
		...
		{
			Common_PlaySfxAtPos(&pthis->hoofSfxPos, NA_SE_EV_HORSE_RUN);
		}
	}
	SkelAnime_Update(&pthis->skin.skelAnime);
	...
	if(pthis->timers[0] == 0)
	{
		...
		else
		{
			...
			pthis->timers[0] = 40;
			Actor_SpawnAsChild(
			    &globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_FHG_FIRE, pthis->actor.world.pos.x, pthis->actor.world.pos.y + 50.0f, pthis->actor.world.pos.z, 0, pthis->actor.shape.rot.y + 0x8000, 0, FHGFIRE_WARP_EMERGE);
...
void EnfHG_Attack(EnfHG* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if(pthis->hitTimer == 0)
		{
			...
			{
				Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_FHG_FIRE, pthis->actor.world.pos.x, (pthis->actor.world.pos.y + 100.0f) + 25.0f, pthis->actor.world.pos.z, 0, 0, 0, FHGFIRE_LIGHTNING_STRIKE);
			}
	...
	if(pthis->hitTimer == 20)
	{
		...
		pthis->spawnedWarp = false;
		Animation_Change(&pthis->skin.skelAnime, &gPhantomHorseLandAnim, -1.0f, 0.0f, Animation_GetLastFrame(&gPhantomHorseLandAnim), ANIMMODE_ONCE, -5.0f);
		pthis->timers[0] = 10;
		pthis->bossGndSignal = FHG_RESET;
	...
	else
	{
		...
		{
			pthis->spawnedWarp = true;
			Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_FHG_FIRE, pthis->inPaintingPos.x, pthis->actor.world.pos.y + 50.0f, pthis->inPaintingPos.z, 0, pthis->actor.shape.rot.y, 0, FHGFIRE_WARP_RETREAT);
			pthis->fhgFireKillWarp = true;
		}
...
void EnfHG_Damage(EnfHG* pthis, GlobalContext* globalCtx)
{
	...
	{
		pthis->spawnedWarp = true;
		Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_FHG_FIRE, pthis->inPaintingPos.x, pthis->actor.world.pos.y + 50.0f, pthis->inPaintingPos.z, 0, pthis->actor.shape.rot.y + 0x8000, 0, FHGFIRE_WARP_RETREAT);
	}
	if(dxz2 == 0.0f)
	{
		BossGanondrof* bossGnd = (BossGanondrof*)pthis->actor.parent;
...
void EnfHG_Retreat(EnfHG* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->timers[0] == 0)
	{
		...
		else
		{
			...
			osSyncPrintf(
			    "ac1 = %x `````````````````````````````````````````````````\n",
			    Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_BOSS_GANONDROF, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, paintingIdxFake + GND_FAKE_BOSS));
		}
	}
}
