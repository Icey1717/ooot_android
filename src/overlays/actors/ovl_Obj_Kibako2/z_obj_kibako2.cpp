...
void ObjKibako2_Break(ObjKibako2* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0, angle = 0; i < 0x10; i++, angle += 0x4E20)
	{
		...
		EffectSsKakera_Spawn(globalCtx, &pos, &velocity, &pos, -200, phi_s0, 28, 2, 0, (Rand_ZeroOne() * 30.0f) + 5.0f, 0, 0, 70, KAKERA_COLOR_NONE, OBJECT_KIBAKO2, gLargeCrateFragmentDL);
	}
...
void ObjKibako2_Idle(ObjKibako2* pthis, GlobalContext* globalCtx)
{
	if((pthis->collider.base.acFlags & AC_HIT) || (pthis->dyna.actor.home.rot.z != 0) || func_80033684(globalCtx, &pthis->dyna.actor) != NULL)
	{
...
void ObjKibako2_Kill(ObjKibako2* pthis, GlobalContext* globalCtx)
{
	...
	if((params & 0x8000) == 0)
	{
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_SW, pthis->dyna.actor.world.pos.x, pthis->dyna.actor.world.pos.y, pthis->dyna.actor.world.pos.z, 0, pthis->dyna.actor.shape.rot.y, 0, params | 0x8000);
	}
