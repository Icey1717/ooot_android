...
void BossDodongo_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &object_kingdodongo_Skel_01B310, &object_kingdodongo_Anim_00F0D8, NULL, NULL, 0);
	...
	switch(pthis->csState)
	{
		...
		case 2:
			...
			if(gSaveContext.eventChkInf[7] & 2)
			{
				if(pthis->unk_198 == 100)
				{
					...
					pthis->unk_198 = 150;
					pthis->unk_204 = 0.0f;
					Animation_Change(&pthis->skelAnime, &object_kingdodongo_Anim_008EEC, 1.0f, 0.0f, Animation_GetLastFrame(&object_kingdodongo_Anim_008EEC), ANIMMODE_ONCE, 0.0f);
		...
		case 3:
			...
			if(fabsf(player->actor.world.pos.x - pthis->actor.world.pos.x) < 200.0f)
			{
				...
				pthis->unk_198 = 0x96;
				pthis->unk_204 = 0.0f;
				Animation_Change(&pthis->skelAnime, &object_kingdodongo_Anim_008EEC, 1.0f, 0.0f, Animation_GetLastFrame(&object_kingdodongo_Anim_008EEC), ANIMMODE_ONCE, -5.0f);
			}
			break;
		case 4:
			...
			Math_SmoothStepToF(&pthis->cameraEye.x, player->actor.world.pos.x + phi_f0 + 70.0f, 0.2f, pthis->unk_204 * 20.0f, 0.0f);
			Math_SmoothStepToF(&pthis->cameraEye.y, player->actor.world.pos.y + 10.0f, 0.2f, pthis->unk_204 * 20.0f, 0.0f);
			Math_SmoothStepToF(&pthis->cameraEye.z, player->actor.world.pos.z - 60.0f, 0.2f, pthis->unk_204 * 20.0f, 0.0f);
			...
			if(pthis->unk_198 == 0x5A)
			{
				if(!(gSaveContext.eventChkInf[7] & 2))
				{
					TitleCard_InitBossName(globalCtx, &globalCtx->actorCtx.titleCtx, SEGMENTED_TO_VIRTUAL(&object_kingdodongo_Blob_017410), 0xA0, 0xB4, 0x80, 0x28);
				}
...
void BossDodongo_SetupDamaged(BossDodongo* pthis)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &object_kingdodongo_Anim_001074, 1.0f, 0.0f, Animation_GetLastFrame(&object_kingdodongo_Anim_001074), ANIMMODE_ONCE, -5.0f);
...
{
	Animation_Change(&pthis->skelAnime, &object_kingdodongo_Anim_00E848, 1.0f, 0.0f, Animation_GetLastFrame(&object_kingdodongo_Anim_00E848), ANIMMODE_ONCE, -5.0f);
...
void BossDodongo_SetupWalk(BossDodongo* pthis)
{
	Animation_Change(&pthis->skelAnime, &object_kingdodongo_Anim_01D934, 1.0f, 0.0f, Animation_GetLastFrame(&object_kingdodongo_Anim_01D934), ANIMMODE_ONCE, -10.0f);
	pthis->unk_1AA = 0;
...
void BossDodongo_SetupBlowFire(BossDodongo* pthis)
{
	...
	pthis->unk_1E4 = 0.0f;
	Animation_Change(&pthis->skelAnime, &object_kingdodongo_Anim_0061D4, 1.0f, 0.0f, Animation_GetLastFrame(&object_kingdodongo_Anim_0061D4), ANIMMODE_ONCE, 0.0f);
...
void BossDodongo_Explode(BossDodongo* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->unk_1DA == 0)
	{
		...
		Animation_Change(&pthis->skelAnime, &object_kingdodongo_Anim_004E0C, 1.0f, 0.0f, Animation_GetLastFrame(&object_kingdodongo_Anim_004E0C), ANIMMODE_ONCE, -5.0f);
...
void BossDodongo_LayDown(BossDodongo* pthis, GlobalContext* globalCtx)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &object_kingdodongo_Anim_0042A8, 1.0f, 0.0f, Animation_GetLastFrame(&object_kingdodongo_Anim_0042A8), ANIMMODE_LOOP, -5.0f);
...
void BossDodongo_Vulnerable(BossDodongo* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->unk_1DA == 0)
	{
		Animation_Change(&pthis->skelAnime, &object_kingdodongo_Anim_009D10, 1.0f, 0.0f, Animation_GetLastFrame(&object_kingdodongo_Anim_009D10), ANIMMODE_ONCE, -5.0f);
...
void BossDodongo_Walk(BossDodongo* pthis, GlobalContext* globalCtx)
{
	...
	Math_SmoothStepToS(&pthis->actor.world.rot.y, Math_FAtan2F(sp48, sp44) * (0x8000 / M_PI), 5, (pthis->unk_1EC * pthis->unk_1E8), 5);
...
void BossDodongo_Roll(BossDodongo* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->unk_1DA == 0)
	{
		...
		if(pthis->actor.bgCheckFlags & BG_STATE_0)
		{
			...
			if(!(pthis->unk_19E & 1))
			{
				Actor_SpawnFloorDustRing(globalCtx, &pthis->actor, &pthis->actor.world.pos, 40.0f, 3, 8.0f, 0x1F4, 0xA, 0);
			}
		}
	}
	...
	Math_SmoothStepToS(&pthis->actor.world.rot.y, Math_FAtan2F(sp4C, sp48) * (0x8000 / M_PI), 5, pthis->unk_1EC * pthis->unk_1E8, 0);
...
void BossDodongo_Update(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(player->actor.world.pos.y < -1000.0f)
	{
		...
		if((pthis->unk_19E & phi_s0_3) == 0)
		{
			...
			EffectSsGMagma2_Spawn(globalCtx, &sp84, &magmaPrimColor_84[magma2DrawMode], &magmaEnvColor_84[magma2DrawMode], 10 - (magma2DrawMode * 5), magma2DrawMode, magmaScale + 100);
		}
...
s32 BossDodongo_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	f32 mtxScaleY;
	f32 mtxScaleZ;
	BossDodongo* pthis = (BossDodongo*)thisx;
...
void BossDodongo_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	{
		POLY_OPA_DISP = Gfx_SetFog(POLY_OPA_DISP, (u32)pthis->colorFilterR, (u32)pthis->colorFilterG, (u32)pthis->colorFilterB, 0, pthis->colorFilterMin, pthis->colorFilterMax);
	}

	Matrix_RotateZ(pthis->unk_23C, MTXMODE_APPLY);
	Matrix_RotateX((pthis->unk_1C4 / 32768.0f) * 3.14159f, MTXMODE_APPLY);

	SkelAnime_DrawOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, BossDodongo_OverrideLimbDraw, BossDodongo_PostLimbDraw, pthis);
...
void BossDodongo_PlayerPosCheck(BossDodongo* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 4; i++)
	{
		...
		if((fabsf(pthis->actor.world.pos.x - temp_v1->x) < 200.0f) && (fabsf(pthis->actor.world.pos.z - temp_v1->z) < 200.0f))
		{
...
{
	Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_BDFIRE, pthis->vec.x, pthis->vec.y - 20.0f, pthis->vec.z, 0, pthis->actor.shape.rot.y, 0, params);
}
...
void BossDodongo_SetupDeathCutscene(BossDodongo* pthis)
{
	pthis->actor.speedXZ = 0.0f;
	pthis->unk_1E4 = 0.0f;
	Animation_Change(&pthis->skelAnime, &object_kingdodongo_Anim_002D0C, 1.0f, 0.0f, Animation_GetLastFrame(&object_kingdodongo_Anim_002D0C), ANIMMODE_ONCE, -5.0f);
...
void BossDodongo_DeathCutscene(BossDodongo* pthis, GlobalContext* globalCtx)
{
	...
	switch(pthis->csState)
	{
		...
		case 5:
			...
			{
				Animation_Change(&pthis->skelAnime, &object_kingdodongo_Anim_003CF8, 1.0f, 0.0f, Animation_GetLastFrame(&object_kingdodongo_Anim_003CF8), ANIMMODE_ONCE, -1.0f);
				pthis->csState = 6;
				Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_BG_BREAKWALL, -890.0f, -1523.76f, -3304.0f, 0, 0, 0, 0x6000);
			}
			break;
		case 6:
			...
			{
				Animation_Change(&pthis->skelAnime, &object_kingdodongo_Anim_00DF38, 1.0f, 30.0f, 59.0f, ANIMMODE_ONCE, -1.0f);
				pthis->csState = 7;
				pthis->unk_228 = 7700.0f;
				pthis->unk_204 = 0.0f;
				pthis->unk_1E4 = 0.0f;
		...
		case 7:
			...
			else
			{
				...
				Math_SmoothStepToS(&pthis->actor.world.rot.y, (Math_FAtan2F(tempSin, tempCos) * (0x8000 / M_PI)), 5, (pthis->unk_1EC * pthis->unk_1E8), 0);
		...
		case 9:
			...
			{
				Animation_Change(&pthis->skelAnime, &object_kingdodongo_Anim_0042A8, 1.0f, 0.0f, (f32)Animation_GetLastFrame(&object_kingdodongo_Anim_0042A8), ANIMMODE_LOOP, -20.0f);
			...
			if(pthis->unk_1DA < 960)
			{
				...
				if(pthis->unk_1DA >= 710)
				{
					...
					}
					{
						...
						func_8002836C(globalCtx, &effectPos, &dustVel_95, &dustAcell_95, &dustPrimColor_95, &dustEnvColor_95, 0x1F4, 0xA, 0xA);
						effectPos.x = Rand_CenteredFloat(120.0f) + pthis->actor.focus.pos.x;
						effectPos.y = -1498.76f;
						effectPos.z = Rand_CenteredFloat(120.0f) + pthis->actor.focus.pos.z;
						colorIndex = (Rand_ZeroOne() * 1.9f);
						EffectSsGMagma2_Spawn(globalCtx, &effectPos, &magmaPrimColor2[colorIndex], &magmaEnvColor2[colorIndex], 10 - (colorIndex * 5), colorIndex, (s16)(Rand_ZeroOne() * 100.0f) + 100);
					}
				}
			}
			else
			...
			if(pthis->csState == 9)
			{
				...
				{
					Vec3f spAC[] = {{-390.0f, 0.0f, -3304.0f}, {-890.0f, 0.0f, -3804.0f}, {-1390.0f, 0.0f, -3304.0f}, {-890.0f, 0.0f, -2804.0f}};

					Vec3f sp7C[] = {{-890.0f, 0.0f, -2804.0f}, {-390.0f, 0.0f, -3304.0f}, {-890.0f, 0.0f, -3804.0f}, {-1390.0f, 0.0f, -3304.0f}};
					Vec3f* sp78;
					s32 pad74;
					...
					Math_SmoothStepToF(&pthis->cameraEye.y, player->actor.world.pos.y + 30.0f, 0.1f, pthis->unk_204 * 20.0f, 0.0f);
			...
			if(pthis->unk_1DA == 820)
			{
				...
				Actor_Spawn(
				    &globalCtx->actorCtx, globalCtx, ACTOR_ITEM_B_HEART, Math_SinS(pthis->actor.shape.rot.y) * -50.0f + pthis->actor.world.pos.x, pthis->actor.world.pos.y,
		...
		case 100:
			if((pthis->unk_1DA < 0x2C6) && (Rand_ZeroOne() < 0.5f))
			{
				...
				EffectSsGMagma2_Spawn(globalCtx, &sp68, &D_808CA568, &D_808CA56C, 5, 1, (s16)(Rand_ZeroOne() * 50.0f) + 50);
			}
			break;
	}
...
void BossDodongo_DrawEffects(GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 80; i++, eff++)
	{
		if(eff->unk_24 == 1)
		{
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_boss_dodongo.c", 5253), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
