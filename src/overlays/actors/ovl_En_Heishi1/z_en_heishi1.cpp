...
static Vec3f sRupeePositions[] = {
    {0.0f, 0.0f, 90.0f}, {-55.0f, 0.0f, 90.0f}, {-55.0f, 0.0f, 30.0f}, {-55.0f, 0.0f, -30.0f}, {0.0f, 0.0f, -30.0f}, {55.0f, 0.0f, -30.0f}, {55.0f, 0.0f, 30.0f}, {55.0f, 0.0f, 90.0f},
};
...
void EnHeishi1_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->path == 3)
	{
		for(i = 0; i < ARRAY_COUNT(sRupeePositions); i++)
		{
			...
			Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_EX_RUPPY, rupeePos.x, rupeePos.y, rupeePos.z, 0, 0, 0, 3);
		}
	}
...
void EnHeishi1_SetupWalk(EnHeishi1* pthis, GlobalContext* globalCtx)
{
	...
	Animation_Change(&pthis->skelAnime, &gEnHeishiWalkAnim, pthis->animSpeed, 0.0f, (s16)frameCount, ANIMMODE_LOOP, pthis->transitionRate);
...
void EnHeishi1_Walk(EnHeishi1* pthis, GlobalContext* globalCtx)
{
	...
	if(!sPlayerIsCaught)
	{
		...
		Math_SmoothStepToS(&pthis->actor.shape.rot.y, (Math_FAtan2F(pathDiffX, pathDiffZ) * (0x8000 / M_PI)), 3, pthis->bodyTurnSpeed, 0);
...
void EnHeishi1_SetupWait(EnHeishi1* pthis, GlobalContext* globalCtx)
{
	...
	Animation_Change(&pthis->skelAnime, &gEnHeishiIdleAnim, pthis->animSpeed, 0.0f, (s16)frameCount, ANIMMODE_LOOP, pthis->transitionRate);
...
void EnHeishi1_Wait(EnHeishi1* pthis, GlobalContext* globalCtx)
{
	...
	if(!sPlayerIsCaught)
	{
		...
		Math_ApproachF(&pthis->headAngle, pthis->headAngleTarget, pthis->headTurnSpeedScale, pthis->headTurnSpeedMax + pthis->headTurnSpeedMax);
			...
			if((sCamDataIdxs[path] == activeCam->camDataIdx) || (sCamDataIdxs[path + 1] == activeCam->camDataIdx))
			{
				if(!sPlayerIsCaught)
				{
					if((pthis->actionFunc == EnHeishi1_Walk) || (pthis->actionFunc == EnHeishi1_Wait))
					{
						...
						EffectSsSolderSrchBall_Spawn(globalCtx, &searchBallPos, &searchBallVel, &searchBallAccel, 2, &pthis->linkDetected);
...
s32 EnHeishi1_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnHeishi1* pthis = (EnHeishi1*)thisx;
