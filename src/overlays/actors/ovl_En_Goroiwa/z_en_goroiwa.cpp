...
s32 EnGoroiwa_MoveDownToNextWaypoint(EnGoroiwa* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.velocity.y < 0.0f && pthis->actor.world.pos.y <= nextPointY)
	{
		if(pthis->bounceCount == 0)
		{
			...
			if(!(pthis->stateFlags & ENGOROIWA_IN_WATER))
			{
				...
				floorY = BgCheck_EntityRaycastFloor5(globalCtx, &globalCtx->colCtx, &floorPoly, &floorBgId, &pthis->actor, &raycastFrom);
	...
	if(pthis->bounceCount == 0 && WaterBox_GetSurfaceImpl(globalCtx, &globalCtx->colCtx, pthis->actor.world.pos.x, pthis->actor.world.pos.z, &ySurface, &waterBox) && pthis->actor.world.pos.y <= ySurface)
	{
...
void EnGoroiwa_SpawnFragments(EnGoroiwa* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0, angle1 = 0; i < 16; i++, angle1 += 0x4E20)
	{
		...
		EffectSsKakera_Spawn(globalCtx, &effectPos, &fragmentVelocity, &effectPos, -340, 33, 28, 2, 0, Rand_ZeroOne() * 7.0f + 1.0f, 1, 0, 70, KAKERA_COLOR_NONE, 1, gBoulderFragmentsDL);
	}
...
static InitChainEntry sInitChain[] = {
    ICHAIN_F32_DIV1000(gravity, -860, ICHAIN_CONTINUE),	  ICHAIN_F32_DIV1000(minVelocityY, -15000, ICHAIN_CONTINUE), ICHAIN_VEC3F_DIV1000(scale, 100, ICHAIN_CONTINUE),
    ICHAIN_F32(uncullZoneForward, 1500, ICHAIN_CONTINUE), ICHAIN_F32(uncullZoneScale, 150, ICHAIN_CONTINUE),	     ICHAIN_F32(uncullZoneDownward, 1500, ICHAIN_STOP),
};
...
void EnGoroiwa_Roll(EnGoroiwa* pthis, GlobalContext* globalCtx)
{
	...
	else if(moveFuncs_81[(pthis->actor.params >> 10) & 1](pthis, globalCtx))
	{
		...
		if(loopMode == ENGOROIWA_LOOPMODE_ONEWAY_BREAK && (pthis->nextWaypoint == 0 || pthis->nextWaypoint == pthis->endWaypoint))
		{
			EnGoroiwa_SpawnFragments(pthis, globalCtx);
		}
		EnGoroiwa_NextWaypoint(pthis, globalCtx);
		if((loopMode == ENGOROIWA_LOOPMODE_ROUNDTRIP) && (pthis->currentWaypoint == 0 || pthis->currentWaypoint == pthis->endWaypoint))
		{
			EnGoroiwa_SetupWait(pthis);
		}
		else if(!((pthis->actor.params >> 10) & 1) && pthis->currentWaypoint != 0 && pthis->currentWaypoint != pthis->endWaypoint)
		{
	...
	if(!(player->stateFlags1 & (PLAYER_STATE1_6 | PLAYER_STATE1_DEAD | PLAYER_STATE1_28 | PLAYER_STATE1_29)))
	{
		...
		switch((pthis->actor.params >> 10) & 1)
		{
			case 1:
				...
				break;
			case 0:
				pthis->actor.floorHeight = BgCheck_EntityRaycastFloor4(&globalCtx->colCtx, &pthis->actor.floorPoly, &sp30, &pthis->actor, &pthis->actor.world.pos);
				break;
		}
