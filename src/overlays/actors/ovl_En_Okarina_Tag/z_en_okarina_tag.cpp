...
void func_80ABEF2C(EnOkarinaTag* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		if((pthis->ocarinaSong != 6) || (gSaveContext.scarecrowSpawnSongSet))
		{
			...
			if((pthis->actor.xzDistToPlayer < (90.0f + pthis->interactRange)) && (fabsf(player->actor.world.pos.y - pthis->actor.world.pos.y) < 80.0f))
			{
				...
				else if((pthis->actor.xzDistToPlayer < (50.0f + pthis->interactRange) && ((fabsf(player->actor.world.pos.y - pthis->actor.world.pos.y) < 40.0f))))
				{
					pthis->unk_15A = 0;
...
void func_80ABF0CC(EnOkarinaTag* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if(pthis->unk_158 != 0)
		{
			if((globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_05) || (globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_06) || (globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_07) || (globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_08) ||
			   (globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_09) || (globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_0A) || (globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_0D))
			{
				if(pthis->switchFlag >= 0)
				{
					Flags_SetSwitch(globalCtx, pthis->switchFlag);
	...
	else if(globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_03)
	{
		...
		switch(pthis->type)
		{
			...
			case 4:
				...
				break;
			case 6:
				globalCtx->csCtx.segment = LINK_IS_ADULT ? SEGMENTED_TO_VIRTUAL(&spot02_scene_Cs_003C80) : SEGMENTED_TO_VIRTUAL(&spot02_scene_Cs_005020);
				gSaveContext.cutsceneTrigger = 1;
