...
static Color_RGB8 sColors[] = {
    {100, 120, 140}, {80, 140, 200}, {100, 150, 200}, {100, 200, 240}, {80, 110, 140}, {70, 160, 225}, {80, 100, 130}, {100, 110, 190},
};

void ObjWarp2block_Spawn(ObjWarp2block* pthis, GlobalContext* globalCtx)
{
	Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DEMO_EFFECT, pthis->dyna.actor.world.pos.x, pthis->dyna.actor.world.pos.y, pthis->dyna.actor.world.pos.z, 0, 0, 0, sSpawnData[(pthis->dyna.actor.params >> 8) & 1].params);

	Actor_Spawn(
	    &globalCtx->actorCtx, globalCtx, ACTOR_DEMO_EFFECT, pthis->dyna.actor.child->world.pos.x, pthis->dyna.actor.child->world.pos.y, pthis->dyna.actor.child->world.pos.z, 0, 0, 0, sSpawnData[(pthis->dyna.actor.child->params >> 8) & 1].params);
}
...
s32 func_80BA1ECC(ObjWarp2block* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->dyna.actor.xzDistToPlayer <= sDistances[(((pthis->dyna.actor.params >> 0xB) & 7))]) || (temp_a3->xzDistToPlayer <= sDistances[(((temp_a3->params >> 0xB) & 7))]))
	{
...
void ObjWarp2block_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	osSyncPrintf("時のブロック(ワープ２) (<arg> %04xH <type> color:%d range:%d)\n", pthis->dyna.actor.params & 0xFFFF, (s32)(pthis->dyna.actor.home.rot.z & 7), (pthis->dyna.actor.params >> 0xB) & 7);
}
...
void func_80BA24F8(ObjWarp2block* pthis, GlobalContext* globalCtx)
{
	...
	while(current != NULL)
	{
		if(current->id == ACTOR_OBJ_WARP2BLOCK && !((current->params >> 0xF) & 1) && ((pthis->dyna.actor.params & 0x3F) == (current->params & 0x3F)))
		{
...
void ObjWarp2block_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_obj_warp2block.c", 584);
	func_80093D18(globalCtx->gfxCtx);

	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_obj_warp2block.c", 588), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, sp44->r, sp44->g, sp44->b, 255);
	gSPDisplayList(POLY_OPA_DISP++, gSongOfTimeBlockDL);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_obj_warp2block.c", 594);
}
