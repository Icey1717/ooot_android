...
void EnWonderTalk2_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->talkMode == 1) && (globalCtx->sceneNum == SCENE_MEN) && (pthis->switchFlag != 0x08) && (pthis->switchFlag != 0x16) && (pthis->switchFlag != 0x2F))
	{
		pthis->unk_15A = false;
		pthis->talkMode = 4;
	}
...
void func_80B3A15C(EnWonderTalk2* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if(!((pthis->actor.xzDistToPlayer > 40.0f + pthis->triggerRange) || (fabsf(player->actor.world.pos.y - pthis->actor.world.pos.y) > 100.0f) || (yawDiff >= 0x4000)))
		{
			if(pthis->unk_158 >= 2)
			{
				osSyncPrintf("\n\n");
...
void func_80B3A4F8(EnWonderTalk2* pthis, GlobalContext* globalCtx)
{
	...
	else if((pthis->talkMode != 4) || !pthis->unk_15A)
	{
		...
		if(((pthis->actor.xzDistToPlayer < (40.0f + pthis->triggerRange)) && (fabsf(player->actor.world.pos.y - pthis->actor.world.pos.y) < 100.0f)) && !Gameplay_InCsMode(globalCtx))
		{
			if(pthis->unk_158 >= 2)
			{
				osSyncPrintf("\n\n");
